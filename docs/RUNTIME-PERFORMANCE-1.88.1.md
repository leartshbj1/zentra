...
The loose Windows build executable was rejected by Code Integrity with error 4551 (policy `0283ac0f-fff1-49ae-ada1-8a933130cad6`). That did not establish that the NSIS-installed application was blocked: Tauri's installer payload is a distinct binary. The isolated Windows installer check in CircleCI job 107 passed, including payload identity, launch, relaunch and SQLite integrity. Its verifier source is `129d008e7e3143f456f16fc848873fec0b763a75`; application bytes still come from job 104 and the application source above.

After backing up 1.88.0, the normal NSIS installer completed on the user's PC (exit 0). The installed 1.88.1 binary exactly matched the payload tested in CircleCI (`d92825f7d99765d7d706bdf2651e60fcf7b6fe60088035eb3dd05a178b31b5c5`). Its launch, relaunch and SQLite integrity checks then passed locally in a separate empty profile. No business data was used, and no Windows security setting or binary was modified to get past the policy. The release gate now checks the shipped installer and installed application, not whether the loose build executable can launch. `desktop/scripts/check-windows-installed-release.py` validates that evidence chain and rejects stale source revisions and loose binaries.

The 14 immutable release assets are published at https://github.com/leartshbj1/zentra/releases/tag/v1.88.1. Both Windows and Mac update channels now offer 1.88.1, and 1.88.1 is installed on this PC. The updater's Zentra signature is not an Authenticode certificate; this successful installation does not promise acceptance by every Windows security policy. Mac remains ad-hoc signed and non-notarized; no physical mobile device or store publication is claimed.
