...
| --- | --- | --- |
| Données partagées | Deux installations et deux collaborateurs retrouvent clients, devis, factures, banque, paie et comptabilité. Modifications simultanées et hors ligne conservées ; conflits explicites ; pas de doublon de numérotation ni de journal déséquilibré. | En cours. Historique initial, envoi reprenable, conflits, preuves comptables, contrôles des états intermédiaires et positions partagées implémentés. Le serveur enregistre maintenant une révision et son reçu dans une opération atomique ; les données originales sont accessibles à une autre session autorisée. Le reçu et ses fragments issus du moteur D1 local passent dans une copie SQLite native isolée. Restent les validations de commandes et cascades, le téléchargement natif authentifié, les fichiers binaires, l’installation récupérable et sa confirmation, la résolution des conflits, l’interface et la recette sur deux appareils. Le schéma natif 61 (format partagé 60) reste non distribué ; la réplication métier complète reste inactive. |
