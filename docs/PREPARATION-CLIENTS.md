...
| --- | --- | --- |
| Données partagées | Deux installations et deux collaborateurs retrouvent clients, devis, factures, banque, paie et comptabilité. Modifications simultanées et hors ligne conservées ; conflits explicites ; pas de doublon de numérotation ni de journal déséquilibré. | En cours. Fichiers de projet distribués en 1.46.1. Serveur initial publié ; copie de 1 365 lignes et cinq références de documents installée dans un second profil de test. Publication native protégée contre les écritures concurrentes, reprise, annulation et interface de publication/import réalisées en développement. 77 tests natifs et 753 tests d'interface passent ; reçu réel du serveur accepté dans un essai dédié. Transactions suivantes, conflits et essais sur deux appareils restent à terminer. Schéma 60 non distribué ; partage métier complet inactif. |
