...
La réception native des révisions suivantes est maintenant raccordée au transport HTTPS authentifié : reprise sur disque, huit fragments maximum par passage, limites de réponse propres à chaque ressource et contrôle de l’entreprise, de la génération et du curseur autour des lectures réseau. Les fichiers sont assemblés hors du fil d’interface et vérifiés contre le manifeste original ; leurs fragments temporaires sont ensuite supprimés. Les données téléchargées restent dans un dossier séparé. Un jeu produit par le moteur D1 passe par cette réception puis par la construction d’une copie SQLite native, sans modifier le profil de travail. L’installation récupérable, la confirmation des transactions locales déjà enregistrées, le planificateur et l’interface restent à terminer avant activation et distribution.
