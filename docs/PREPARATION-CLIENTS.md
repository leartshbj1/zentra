...
| --- | --- | --- |
| Données partagées | Deux installations et deux collaborateurs retrouvent clients, devis, factures, banque, paie et comptabilité. Modifications simultanées et hors ligne conservées ; conflits explicites ; pas de doublon de numérotation ni de journal déséquilibré. | En cours. La synchronisation des fichiers de projet est distribuée en 1.46.1. Le schéma 60 en préparation ajoute une capture transactionnelle locale et une restauration sans reprise de l'ancienne génération. L'initialisation serveur, les droits par données, le transport et le traitement des conflits restent à réaliser ; la réplication complète est inactive. |
