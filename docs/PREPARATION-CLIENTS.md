...
| --- | --- | --- |
| Données partagées | Deux installations et deux collaborateurs retrouvent clients, devis, factures, banque, paie et comptabilité. Modifications simultanées et hors ligne conservées ; conflits explicites ; pas de doublon de numérotation ni de journal déséquilibré. | En cours. Fichiers de projet distribués en 1.46.1. Publication atomique du premier historique et lectures authentifiées disponibles côté serveur. Reprise initiale en préparation : les positions exactes des événements sont conservées ; 1 365 lignes et cinq références de documents produites nativement, publiées dans la base de test puis reçues et installées dans un second profil. Création de client, suite des mouvements de stock, sauvegarde de sécurité et refus d’écraser un profil occupé vérifiés ; 518 tests serveur et 73 tests natifs de synchronisation passent. Commande de publication native, interface, changements postérieurs à la préparation, transactions suivantes et conflits restent à terminer. Natif non distribué ; partage métier complet inactif. |
...
| Publication | Sources testées, site déployé, installateurs actuels vérifiés ; installation neuve et mise à jour préservant les données. | Version native 1.46.1 publiée ; site 90 déployé, nouveaux accès d'historique vérifiés sans authentification (401 et absence de cache). Publication initiale, réception et import dans un profil neuf testés localement ; leur recette HTTPS authentifiée reste à faire. Installation neuve et remplacement Windows réussis ; remplacement Mac réussi avec autorisation normale du Trousseau, données et identité conservées. Signatures d'éditeur et notarisation restent à obtenir. Le schéma 60 et son parcours natif de partage ne sont pas distribués. |
