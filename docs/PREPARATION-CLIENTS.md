...
| --- | --- | --- |
| Données partagées | Deux installations et deux collaborateurs retrouvent clients, devis, factures, banque, paie et comptabilité. Modifications simultanées et hors ligne conservées ; conflits explicites ; pas de doublon de numérotation ni de journal déséquilibré. | En cours. Fichiers de projet distribués en 1.46.1. Serveur 89 publié : publication atomique du premier historique, bornes de numérotation et lecture authentifiée des fichiers d'origine. Recette locale : historique natif de 1 365 lignes et cinq références de documents publié dans la base de test puis reçu dans le stockage isolé d'un second profil natif, coupure et empreintes contrôlées. 511 tests serveur, 70 tests natifs de synchronisation et neuf de compte passent ; la publication passe aussi dans D1. La recette HTTPS fournisseur de 1 766 lignes reste celle du site 88. Import dans la base de travail, ordre portable des événements, interface, transactions suivantes et conflits restent à terminer. Réception native non distribuée ; partage métier complet inactif. |
...
| Publication | Sources testées, site déployé, installateurs actuels vérifiés ; installation neuve et mise à jour préservant les données. | Version native 1.46.1 publiée ; site 89 déployé, nouveaux accès d'historique vérifiés sans authentification (401 et absence de cache). Publication initiale et réception testées localement ; leur recette HTTPS authentifiée reste à faire. Installation neuve et remplacement Windows réussis ; remplacement Mac réussi avec autorisation normale du Trousseau, données et identité conservées. Signatures d'éditeur et notarisation restent à obtenir. Le schéma 60 et son transport natif ne sont pas distribués. |
