...
Les tests couvrent les vrais XML TVA et ZIP de clôture produits par l'application, leur restauration dans un profil isolé, les registres inchangés, les fichiers absents ou altérés, la réparation d'un export local endommagé, le retour arrière, les limites et la compatibilité du format 1. La recette du coffre HTTPS avec ces nouveaux exports a réussi comme décrit ci-dessous ; l'essai sur un second appareil physique et la distribution de ce code natif restent à effectuer.
...
La recette du format 2 et de la reprise par confirmations a réussi contre la **version 76 du site**, source `3ab230b6de10da3e57f2dd84220fd14c927ff0c3`, configuration inchangée (révision 23). La source courante passe **300 tests serveur, un essai optionnel ignoré**, dont 22 tests du coffre ; **5 tests natifs ciblés réussissent**, un essai HTTPS étant exécuté séparément. TypeScript, lint, compilation du service et Clippy sur toutes les cibles sans avertissement passent.

L'essai natif HTTPS réussit en **136,07 secondes**. L'archive `74ea8b30-4365-4988-b09c-aac53032e880` contient 9 534 534 octets, en deux fragments, avec l'empreinte `dabd46cf603b8c36f6133346d0bbdde9b0268c13bb31e765cafdfa178fb3797d`. Après le premier fragment, la réouverture du profil envoie seulement le second et ignore le premier. La simulation d'une confirmation locale perdue ne renvoie aucun fragment. Le second profil restaure quatre fichiers : document de projet réellement importé, remplissage fictif, XML TVA et ZIP de clôture réellement générés. Le lien au projet et les trois registres restent identiques. Les deux profils ont des identités d'installation distinctes sur un seul ordinateur physique.

La recette a supprimé uniquement sa propre archive et révoqué ses deux sessions. D1 confirme l'état `deleted` ; les deux reçus de fragments restent volontairement comme métadonnées. Le compte affiche zéro appareil actif et la récupération web n'affiche aucune sauvegarde complète dans l'entreprise fictive. Les trois API du coffre refusent les lectures anonymes (401). Les installateurs publics restent en **1.46.1, schéma 59** ; ni le nouveau format natif ni la réplication métier complète ne sont encore distribués.
