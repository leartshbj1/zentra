...
| Connexion depuis le paquet Windows 1.45 | Deux installations isolées ont accepté une licence Start signée après approbation : titulaire puis collaborateur. La connexion personnelle du collaborateur et sa licence restent valides après redémarrage | Contrôle sur Windows ; ne prouve pas ce parcours complet sur un téléphone physique |
| Paiement et accès collaborateurs | Parcours Start réalisé avec Stripe sandbox : Checkout à 59 CHF, événements `checkout.session.completed` et `invoice.paid` traités dans D1, association de l'entreprise, invitation nominative, connexion Supabase, approbation native et licence valide | Titulaire connecté via son accès Sites existant ; identité Supabase du collaborateur préparée sans envoi d'e-mail. Les parcours Checkout complets Solo et Pro n'ont pas été rejoués |
...
2. Étendre la recette aux Checkout Solo et Pro et à un titulaire utilisant sa connexion Supabase. Les montants et quotas des trois formules sont déjà contrôlés par les tests automatisés ; la recette Stripe et Windows complète ci-dessus concerne Start. Ne pas utiliser de paiement réel pour cette recette.
