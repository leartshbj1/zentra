...
État de la comparaison : 2 septembre 2026. Version source documentée : Zentra 1.17 source, sans publication annoncée.
...
| --- | --- | --- |
| Socle local | installation Windows, configuration guidée, SQLite locale, sauvegardes, audit, mises à jour signées | Disponible ; onboarding progressif après confirmation de l'identité et de l'activité, avec réglages de facturation, temps/coûts et sauvegarde différés jusqu'à leur confirmation explicite en 1.15 source ; logo importé dans le stockage local géré et référencé par hash en 1.17 ; identité visuelle Zentra et migration de l'ancienne installation conservant les données ; guide relançable et updater avec reprise guidée en 1.10 ; Authenticode reste à acquérir |
...
| Paie suisse | employés, cotisations versionnées, fiches, import OCR local des documents de paie, écritures | Analyse locale multipage, double lecture visuelle, corroboration du texte PDF quand il existe, manifeste d'audit v2, rapprochement salarié Unicode et confirmation humaine ; PDF A4 professionnel ; date réglementaire, arrondi au CHF 0.05, garde LPP 2026, contrôles documentés AAP/AANP/CAF et cumul LAA dérivé de l'ouverture et des fiches antérieures en 1.17 ; QST autonome, moteur LPP multiannuel complet, certificat annuel, ELM et certification Swissdec non livrés |
...
- Aucun OCR d'achats n'est livré, y compris dans la source 1.17. L'OCR local existant est réservé à l'import de documents de paie. Le socle expérimental d'analyse des factures fournisseurs n'est relié ni à l'interface, ni à la base, ni au moteur comptable et ne doit pas être présenté comme une fonction disponible.
...
## Périmètre documentaire et paie de Zentra 1.13, complété en 1.17 source
