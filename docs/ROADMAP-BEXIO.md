...
| --- | --- | --- |
| Socle local | installation Windows et macOS, configuration guidée, SQLite locale, sauvegardes, audit, mises à jour signées | Installateur Windows et DMG macOS universel disponibles ; onboarding progressif après confirmation de l'identité et de l'activité, puis checklist 1.18 calculée exclusivement depuis les vraies données locales ; logo importé dans le stockage local géré et référencé par hash en 1.17 ; identité visuelle Zentra et migration de l'ancienne installation Windows conservant les données ; guide relançable et updater avec reprise guidée en 1.10. Le DMG macOS est publié en accès anticipé avec signature ad hoc ; Developer ID/notarisation et Authenticode restent à acquérir |
...
| Achats | fournisseurs, commandes, réceptions, factures et avoirs fournisseurs, justificatifs, rapprochement, échéances et paiements | Disponible : commande → réception partielle/complète → facture → rapprochement multi-commandes → paiement/comptabilité, avec tolérance globale, protection des ventilations et avoir distinct ; en 1.19, import déterministe d'un e-mail `.eml`/`.txt` en brouillon local, toujours revu avant validation |
...
- Zentra 1.19 analyse localement les en-têtes et le texte MIME d'un message `.eml`/`.txt` choisi par l'utilisateur. Il propose un brouillon, signale les doublons et laisse obligatoires la catégorie, la vérification puis la validation. Il ne se connecte pas encore directement à Gmail/Outlook et ne lit pas le contenu du PDF joint ; toute valeur absente reste à saisir.
...
1. Ajouter un connecteur facultatif Gmail/Outlook côté serveur avec consentement OAuth révocable, puis extraire localement le contenu des PDF joints avec double lecture et contrôle humain ; l'import de messages exportés est déjà relié au brouillon fournisseur en 1.19.
