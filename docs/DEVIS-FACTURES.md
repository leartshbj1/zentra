...
Lorsque la comptabilité est inactive, le parcours l’indique et conserve le fonctionnement existant des règlements à comptabiliser. Aucun calcul fiscal ni schéma de base n’est changé. Recette : `desktop/tests/customer-settlement-guided-journey.mjs` et `desktop/tests/customer-credit-settlement-journey.mjs`. Ces changements sont inclus dans la préversion Windows 1.60.0 ; ses contrôles et leurs limites sont décrits dans [la note de livraison](RELEASE-WINDOWS-1.60.0.md).
