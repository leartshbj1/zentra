...
La version `1.23.0` ajoute l’import de catalogues fournisseurs Excel/CSV/TSV, les modèles de texte de pied de document, la création de contacts pendant un devis ou une facture, les acomptes en pourcentage et les corrections traçables de documents émis. Elle permet aussi de modifier, supprimer ou annuler les devis selon leur état, trie les documents du plus récent au plus ancien et réduit les demandes répétées du trousseau macOS grâce à un cache protégé en mémoire et à des écritures sérialisées. Le canal de mise à jour reste signé pour Windows et macOS. Le build macOS universel est distribué en accès anticipé avec une signature ad hoc, sans certificat Apple ni notarisation; Gatekeeper peut donc demander une autorisation manuelle au premier lancement. La signature de mise à jour Tauri protège les versions téléchargées par l’application, mais ne remplace pas la signature Developer ID attendue par macOS.
