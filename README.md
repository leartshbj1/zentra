...
La version source `1.17.0` ajoute le logo d'entreprise local immuable dans les documents, un import de paie audité par manifeste v2 avec corroboration du texte PDF et rapprochement Unicode, une fiche de salaire A4 remaniée, les contrôles documentés AAP/AANP/CAF et le cumul LAA dérivé des fiches antérieures. Elle renforce aussi le passage d'un paiement client vers la comptabilité : date explicite, facture émise active, solde contrôlé et écriture vérifiable. Le modèle SmolVLM reste générique, local et soumis à une validation humaine ; il n'est pas présenté comme un moteur suisse certifié. La version publique n’est remplacée qu’après recette de l’installateur et publication explicite. Avant une diffusion commerciale générale, signer aussi l’exécutable et l’installateur avec un certificat Authenticode horodaté. La licence augmente fortement le coût d’un partage ou d’une modification non autorisée, mais aucun logiciel exécuté sur un ordinateur contrôlé par l’utilisateur ne peut être garanti absolument incrackable.
