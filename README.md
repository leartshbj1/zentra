...
La version `1.22.0` renforce la conservation locale des licences, l’import contrôlé des factures fournisseurs, les exports et la navigation depuis l’agenda. Le jeton de licence n’est plus conservé dans SQLite : il est placé dans le stockage protégé du système, avec une migration des installations existantes qui échoue de manière sûre si le coffre est absent ou incohérent. Les imports `.eml` avec pièce justificative sont traités comme une seule opération vérifiable, et les exports JSON ou CSV excluent explicitement les secrets et les fichiers binaires. Le canal de mise à jour reste signé pour Windows et macOS. Le build macOS universel est distribué en accès anticipé avec une signature ad hoc, sans certificat Apple ni notarisation; Gatekeeper peut donc demander une autorisation manuelle au premier lancement. La signature de mise à jour Tauri protège les versions téléchargées par l’application, mais ne remplace pas la signature Developer ID attendue par macOS.
