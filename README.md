...
La version `1.24.0` fiabilise l’import de catalogues fournisseurs Excel/CSV/TSV, les dates et taux TVA des devis et factures, les acomptes, les corrections en chaîne, l’annulation des devis acceptés, les relances, les factures fournisseurs et le journal comptable manuel. Les opérations sensibles rejouées après une interruption sont protégées contre les doublons. L’import salarial local utilise une lecture rapide et déterministe des valeurs explicitement libellées dans les PDF numériques, complétée par SmolVLM et toujours soumise à la validation humaine; un scan que le modèle ne peut pas attribuer avec certitude est refusé sans créer de brouillon. La version étend aussi les tests de non-régression du trousseau macOS. Le canal de mise à jour reste signé pour Windows et macOS. Le build macOS universel est distribué en accès anticipé avec une signature ad hoc, sans certificat Apple ni notarisation; Gatekeeper peut donc demander une autorisation manuelle au premier lancement. La signature de mise à jour Tauri protège les versions téléchargées par l’application, mais ne remplace pas la signature Developer ID attendue par macOS.
