...
La version `1.20.1` ajoute un canal de mise à jour intégré et signé pour Windows et macOS, tout en conservant l’agenda local, l’import contrôlé d’un e-mail fournisseur exporté en `.eml` ou en texte et les fondations du compte d’entreprise multi-utilisateur introduits dans la série 1.19. L’analyse utilise des règles déterministes locales, sans IA et sans connexion directe à la boîte mail. Elle crée seulement un brouillon : l’utilisateur confirme le fournisseur, l’échéance, la catégorie, le règlement et la comptabilisation. Le build macOS universel reste publié en accès anticipé avec une signature ad hoc, sans certificat Apple ni notarisation. Gatekeeper peut donc demander une autorisation manuelle au premier lancement. La signature de mise à jour Tauri protège les versions suivantes téléchargées par l’application, mais ne remplace pas la signature Developer ID attendue par macOS. Un certificat Developer ID Application et la notarisation Apple supprimeront cette étape supplémentaire.
