...
Zentra 1.20 réunit deux produits dans ce dépôt :
...
La version `1.21.0` fiabilise l’agenda local et l’import contrôlé d’une facture reçue dans un e-mail exporté en `.eml`. Zentra extrait désormais une vraie pièce PDF ou image encodée en Base64, vérifie ses octets et son empreinte, la rattache au brouillon et bloque les reprises en double. Les rendez-vous résistent aux rejeux et aux modifications concurrentes, se mettent à jour au changement de jour et ouvrent la bonne échéance métier. L’analyse reste déterministe, locale, sans IA et sans connexion directe à la boîte mail; l’utilisateur garde la validation comptable. Le canal de mise à jour reste signé pour Windows et macOS. Le build macOS universel est distribué en accès anticipé avec une signature ad hoc, sans certificat Apple ni notarisation; Gatekeeper peut donc demander une autorisation manuelle au premier lancement. La signature de mise à jour Tauri protège les versions téléchargées par l’application, mais ne remplace pas la signature Developer ID attendue par macOS.
