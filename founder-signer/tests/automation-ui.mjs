...
try{for(const viewport of [{width:1140,height:850},{width:820,height:650}]){
 ...
 await page.addInitScript(()=>{
  const f=JSON.parse(sessionStorage.getItem('automation-fixture')||'{"records":{},"pending":null,"operations":[],"writes":0}');window.fixture=f;
  const save=()=>sessionStorage.setItem('automation-fixture',JSON.stringify(f));
  ...
  window.__TAURI__={core:{invoke:async(command,args)=>{
   ...
   if(a.product==='automation'&&a.operation==='grant'&&(!a.organizationId||a.plan))throw Error('Company missing or unexpected plan');
   if(a.operation==='revoke'&&a.plan)throw Error('Unexpected plan');
   f.pending=a;save();
   if(!f.operations.includes(a.operationId)){const old=f.records[key(a)];f.operations.push(a.operationId);f.writes++;f.records[key(a)]={email:a.email,product:a.product,plan:a.plan||old?.plan,organizationId:a.organizationId||old?.organizationId,status:a.operation==='revoke'?'revoked':'pending',note:a.note,revision:(old?.revision||0)+1,expiresAt:new Date(Date.now()+14*86400000).toISOString(),accountLinked:false};}
 ...
 await choose('automation');await lookup();await page.evaluate(()=>window.fixture.loseResponse=true);await page.locator('#grant').click();await page.locator('#pending').waitFor();assert(await page.locator('#product').isDisabled());
 await page.reload();await ready();assert.equal(await page.locator('#product').inputValue(),'automation');await page.locator('#retry').click();await ready();await check();assert.equal(await page.locator('#automation-company').inputValue(),'org_two');assert.equal(await page.evaluate(()=>window.fixture.writes),5);
 await page.locator('#automation-company').selectOption('org_one');assert(await page.locator('#reassign-area').isVisible());assert(await page.locator('#grant').isDisabled());await page.locator('#reassign').click();await ready();await check();assert.equal(await page.locator('#automation-company').inputValue(),'org_one');assert.match(await page.locator('#success-title').textContent(),/Entreprise corrigée/);assert(await page.locator('#reassign-area').isHidden());
 await page.locator('#revoke').click();await page.locator('#confirm-revoke').click();await ready();await check();assert.equal(await page.evaluate(()=>window.fixture.records['automation:demo@example.invalid'].status),'revoked');assert.equal(await page.evaluate(()=>window.fixture.records['zentra:demo@example.invalid'].status),'pending');
 await page.locator('#show-all').check();await page.locator('.record').click();await ready();await page.screenshot({path:output+'/automation-ui-'+viewport.width+'.png',fullPage:true});
 assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth),false);assert.deepEqual(errors,[]);report.push({viewport,allDurations:true,companyChoice:true,productIsolation:true,pendingRecovery:true,revoke:true,errors});await page.close();
}}finally{await browser.close();server.close();}
await writeFile(output+'/automation-ui-results.json',JSON.stringify(report,null,2));console.log(JSON.stringify(report));
