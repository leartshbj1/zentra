...
export async function startServer(root,backend,options={}){
  ...
  const server=createServer(async(req,res)=>{
    ...
    if(url.pathname==='/health'&&req.method==='GET'&&req.headers['x-launch-token']===launchToken){lastSeen=Date.now();return json(200,{app:'ZentraFondateur',version:'1.4.2',pid:process.pid});}
    ...
    try{let content=await readFile(path.join(root,'ui',name));if(name.endsWith('.html'))content=Buffer.from(content.toString('utf8').replace(/<script src="(access|app|keys)\.js"/,'<script src="bridge.js" defer></script><script src="$1.js"').replace(/Zentra Fondateur 1\.\d+(?:\.\d+)?/g,'Zentra Fondateur 1.4.2'));
