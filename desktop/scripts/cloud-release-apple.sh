...
test)
  ...
  pnpm --dir desktop exec vitest run src/companyAccount.test.ts src/companyRealtime.test.ts src/projectSyncScheduler.test.ts src/automationCompanySession.test.ts src/appReleaseNotes.test.ts src/automationDailySummary.test.tsx src/automationHub.test.tsx src/automationJournal.test.tsx src/automationNavigation.test.tsx src/appearance.test.ts src/supplierInboxReview.test.ts src/supplierInboxBatch.test.ts src/languageCatalogCoverage.test.ts src/projectReport.test.ts \
  ...
  cargo test --manifest-path desktop/src-tauri/Cargo.toml --locked --lib automation::tests -- --test-threads=1 2>&1 | tee desktop/artifacts/validation/automation-tests-macos.log
