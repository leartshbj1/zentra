...
export function FixedAssetsPanel({workspace,readOnly,onChanged,onSetup}:{workspace:Workspace;readOnly:boolean;onChanged:()=>Promise<unknown>;onSetup:()=>void}){
  ...
  async function run(action:()=>Promise<{items:FixedAssetRow[]}>){
    ...
    try{const result=await action();setItems(result.items);setLoaded(true);setEditing(false);setReview(null);await onChanged();}
  ...
  function start(){setDraft({...blank(),assetAccountId:assets[0]?.id||'',depreciationAccountId:depreciation[0]?.id||'',counterpartAccountId:workspace.accountingSettings?.expenseAccountId||''});setCost('');setResidual('0');setRate('20');setConfirmed(false);setEditing(true);setError('');requestAnimationFrame(()=>heading.current?.focus());}
  ...
  return <section className="panel fixed-assets">
    <header className="fixed-assets__header"><div><h2>Immobilisations</h2><p>Vos équipements, leur valeur et leurs amortissements.</p></div><Button type="button" disabled={busy||readOnly||!enabled} onClick={start}><Plus size={18}/>Ajouter un bien</Button></header>
