...
function BackupStep({ settings, setSettings, privacyConfirmed, setPrivacyConfirmed, issues, onError }: { settings: AppSettings; setSettings: SettingsSetter; privacyConfirmed: boolean; setPrivacyConfirmed: (value: boolean) => void; issues: IssueMap; onError: (message: string) => void }) {
  ...
  return (
    <div>
      <StepHeader eyebrow="Étape 5 sur 5" title="Sauvegarde et confidentialité" text={isMobileRuntime() ? 'La base reste sur cet appareil. Enregistrez une copie de sauvegarde dans un emplacement distinct grâce au partage du système.' : 'La base active reste sur cet ordinateur. Une sauvegarde externe protège votre entreprise d’une panne ou d’un vol.'} />
      <div className="privacy-banner"><DatabaseIcon size={24} /><div><strong>Données métier locales</strong><p>Clients, projets, temps, pièces jointes et salaires restent dans la base locale. Si vous reliez un compte, les PDF des factures émises sont aussi conservés dans le coffre Zentra pendant dix ans.</p></div></div>
      {isMobileRuntime() ? <div className="privacy-banner"><FolderOpen size={24} /><div><strong>Enregistrer ou partager une sauvegarde</strong><p>Depuis les paramètres, créez une sauvegarde puis choisissez Fichiers, votre espace cloud ou une application compatible. Aucun dossier système n’est à configurer ici.</p></div></div> : <div className="form-grid setup-form"><Field label="Dossier pour les sauvegardes manuelles" hint="Une sauvegarde ne sera créée que lorsque vous utiliserez le bouton Sauvegarder." required wide error={issues['backup.folder']}><div className="path-picker"><input aria-invalid={Boolean(issues['backup.folder'])} readOnly value={backup.folder} /><Button data-field="backup.folder" type="button" variant="secondary" disabled={choosing} onClick={() => void chooseFolder()}>{choosing ? <LoaderCircle className="spin" size={16} /> : <FolderOpen size={16} />} {choosing ? 'Ouverture…' : 'Choisir'}</Button></div></Field></div>}
