...
export type MailState = { scope: string; connection: Partial<Omit<MailConnection, 'password'>> & { connected: boolean }; templates: MailTemplates; signature?: MailSignature; companyLogoDataUrl?: string | null; companyLogoError?: string | null; canConfigure: boolean };
...
export type MailPreview = { scope: string; target: MailTarget; sourceRevision: string; recipient: string; subject: string; body: string; signatureLogoDataUrl?: string | null; signatureLogoError?: string | null; attachmentName: string; history: Array<{ recipient: string; subject: string; status: 'pending' | 'accepted' | 'rejected' | 'uncertain'; createdAt: string }> };
...
export const outgoingMail = {
  ...
  saveTemplates: (scope: string, templates: MailTemplates, signature?: MailSignature) => invoke<void>('save_outgoing_mail_templates', { scope, templates, ...(signature ? { signature } : {}) }),
