...
it('distinguishes initial loading, disconnected companies and a temporary outage', () => {
  ...
  mock.company.status = 'unavailable'; expect(render()).toContain('reviendra automatiquement'); expect(render()).not.toContain('15 CHF');
});
...
it('localizes the hub, statuses, workflows and settings into all supported languages', () => {
  const messages = [...Object.values(readinessLabels), ...automationWorkflows.flatMap(flow => [flow.title, flow.description])];
  for (const file of ['AutomationHub.tsx', 'AutomationSettings.tsx', 'AutomationTools.tsx']) {
