...
function ExpensesScreen({ workspace, query, onCreate, onEdit, onArchive }: { workspace: Workspace; query: string; onCreate: () => void; onEdit: (item: Expense) => void; onArchive: (item: Expense) => void }) {
  ...
  const expenseBlock = creationBlockReason('expenses', {
    clients: workspace.clients.length,
    projects: workspace.projects.length,
    trackableProjects: workspace.projects.filter((project) => project.status !== 'closed').length,
    activeEmployees: workspace.employees.filter((employee) => employee.active).length,
  ...
  if (!workspace.expenses.length) return <EmptyState icon={<WalletCards />} title="Aucune dépense" text={expenseBlock || 'Enregistrez uniquement les achats et coûts réellement engagés.'} actionLabel={expenseBlock ? undefined : 'Enregistrer une dépense'} onAction={expenseBlock ? undefined : onCreate} />;
