...
type View = 'dashboard' | 'projects' | 'clients' | 'quotes' | 'invoices' | 'reminders' | 'time' | 'team' | 'expenses' | 'reports' | 'accounting' | 'settings';
type ModalState =
...
export function WorkspaceApp({ workspace, setWorkspace, readOnly = false }: { workspace: Workspace; setWorkspace: Dispatch<SetStateAction<Workspace | null>>; readOnly?: boolean }) {
  ...
  const overdue = workspace.invoices.filter((invoice) => invoice.type !== 'credit_note' && invoice.status === 'issued' && invoice.dueDate && invoice.dueDate < todayIso());
    ...
    <div className="desktop-app">
      ...
      <main className="app-main">
        <header className="topbar">
          ...
          <div className="topbar__tools">{searchableView ? <label className="global-search"><Search size={16} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Rechercher dans cette vue" /></label> : null}<div className="company-avatar">{settings.organization.legalName.slice(0, 2).toUpperCase()}</div></div>
        </header>
        ...
        <section className="page-content">
          ...
          {view === 'quotes' ? <DocumentsScreen entity="quotes" workspace={workspace} query={search} busy={busy} onEdit={(item) => setModal({ type: 'document', entity: 'quotes', item })} onCreate={() => setModal({ type: 'document', entity: 'quotes' })} onIssue={(item) => void act(() => desktopApi.issueDocument('quotes', item.id, item.issueDate, item.validUntil), 'Le devis a été émis et numéroté.', false)} onStatus={(item, status) => void act(() => desktopApi.updateQuoteStatus(item.id, status), status === 'accepted' ? 'Le devis a été marqué accepté.' : status === 'refused' ? 'Le devis a été marqué refusé.' : 'Le devis a été marqué expiré.', false)} onConvert={(item) => void act(() => desktopApi.convertQuote(item), 'Une facture brouillon unique a été créée à partir du devis accepté.', false)} onPrint={(item) => setPrintTarget({ entity: 'quotes', value: item })} onArchive={(item) => void archive('quotes', item.id, item.title)} /> : null}
          ...
          {view === 'team' ? <TeamScreen workspace={workspace} query={search} onCreateEmployee={() => setModal({ type: 'employee' })} onEditEmployee={(item) => setModal({ type: 'employee', item })} onCreatePayslip={() => setModal({ type: 'payslip' })} onEditPayslip={(item) => setModal({ type: 'payslip', item })} onPostPayslip={(item) => void act(() => desktopApi.postPayslip(item.id), 'La fiche a été comptabilisée et verrouillée.', false)} onPrint={(item) => setPrintTarget({ entity: 'payslips', value: item })} onArchiveEmployee={(item) => void archive('employees', item.id, item.name)} onArchivePayslip={(item) => void archive('payslips', item.id, `Fiche ${item.period}`)} /> : null}
      ...
      {modal ? <WorkspaceModal state={modal} workspace={workspace} busy={busy} close={() => setModal(null)} act={act} onQrReady={(invoice, qr) => { setModal(null); setPrintTarget({ entity: 'invoices', value: invoice, qr }); }} /> : null}
      {printTarget ? <PrintSheet target={printTarget} workspace={workspace} onClose={() => setPrintTarget(null)} /> : null}
    </div>
  );
}

function CreateButton({ view, onClick, terminology }: { view: View; onClick: Dispatch<SetStateAction<ModalState>>; terminology: ReturnType<typeof projectTerminology> }) {
...
function DocumentsScreen(sourceProps: DocumentsProps) {
  ...
  if (entity === 'quotes') {
    ...
    return <div className="panel table-panel"><table><thead><tr><th>Document</th><th>Client</th><th>Date</th><th>Montant TTC</th><th>Statut</th><th aria-label="Actions" /></tr></thead><tbody>{filtered.map((document) => {
      ...
      return <tr key={quote.id}><td><div className="document-cell"><span><FileCheck2 size={16} /></span><div><strong>{quote.number || 'Numéro attribué à l’émission'}</strong><small>{quote.title}</small></div></div></td><td><strong className="table-subtle">{client?.company || client?.name || '—'}</strong></td><td><span>{formatDate(quote.issueDate)}</span><small>Valable au {formatDate(quote.validUntil)}</small></td><td><strong>{formatMoney(documentTotals(quote.lines).totalCents)}</strong></td><td><StatusBadge status={quote.status} />{converted ? <small>Facture créée</small> : null}</td><td><div className="document-actions"><Button variant="ghost" size="icon" onClick={() => props.onEdit(quote)} title={quote.status === 'draft' ? 'Modifier' : 'Consulter'}><Pencil size={15} /></Button>{quote.status === 'draft' ? <><Button variant="ghost" size="icon" disabled={busy || !quote.lines.length} onClick={() => props.onIssue(quote)} title="Émettre"><CheckCircle2 size={16} /></Button><Button variant="ghost" size="icon" onClick={() => props.onArchive(quote)} title="Supprimer le brouillon"><Archive size={15} /></Button></> : null}{quote.status === 'issued' ? <><Button variant="ghost" size="icon" disabled={busy} onClick={() => props.onStatus(quote, 'accepted')} title="Marquer accepté"><CheckCircle2 size={16} /></Button><Button variant="ghost" size="icon" disabled={busy} onClick={() => props.onStatus(quote, 'refused')} title="Marquer refusé"><X size={16} /></Button><Button variant="ghost" size="icon" disabled={busy} onClick={() => props.onStatus(quote, 'expired')} title="Marquer expiré"><Clock3 size={16} /></Button></> : null}{quote.status === 'accepted' && !converted ? <Button variant="ghost" size="icon" disabled={busy} onClick={() => props.onConvert(quote)} title="Créer une facture unique"><ArrowRight size={16} /></Button> : null}{quote.status !== 'draft' ? <Button variant="ghost" size="icon" onClick={() => props.onPrint(quote)} title="Imprimer"><Printer size={15} /></Button> : null}</div></td></tr>;
...
function SettingsScreen({ workspace, busy, setBusy, onWorkspace, onNotice }: { workspace: Workspace; busy: boolean; setBusy: (value: boolean) => void; onWorkspace: Dispatch<SetStateAction<Workspace | null>>; onNotice: (value: { tone: 'success' | 'error'; text: string } | null) => void }) {
  ...
  async function chooseLogo() {
    const logoPath = await desktopApi.chooseLogo();
    if (!logoPath) return;
    const next = { ...settings, organization: { ...settings.organization, logoPath } };
    setSettings(next);
    await execute(() => desktopApi.saveSettings(next), 'Le logo a été enregistré pour les documents.');
  }
...
function WorkspaceModal({ state, workspace, busy, close, act, onQrReady }: { state: Exclude<ModalState, null>; workspace: Workspace; busy: boolean; close: () => void; act: (action: () => Promise<Workspace>, message: string, close?: boolean) => Promise<boolean>; onQrReady: (invoice: Invoice, qr: StoredSwissQrBill) => void }) {
...
function EmployeeForm({ item, busy, close, act }: { item?: Employee; busy: boolean; close: () => void; act: ActionRunner }) {
  ...
  return <Modal title={item ? 'Modifier le collaborateur' : 'Nouveau collaborateur'} description="Aucun salaire, taux ou coût n’est prérempli." onClose={close} wide><form onSubmit={submitForm(async (form) => {
    const data = { employeeNumber: String(form.get('employeeNumber')), name: String(form.get('name')), role: String(form.get('role')), email: String(form.get('email')), phone: String(form.get('phone')), addressLine1: String(form.get('addressLine1')), addressLine2: String(form.get('addressLine2')), postalCode: String(form.get('postalCode')), city: String(form.get('city')), canton: String(form.get('canton')), country: String(form.get('country')).trim().toUpperCase(), birthDate: String(form.get('birthDate')), socialSecurityNumber: String(form.get('avsNumber')), iban: String(form.get('iban')), employmentStartDate: String(form.get('employmentStart')), employmentEndDate: String(form.get('employmentEnd')), employmentRate: numberFromInput(form.get('employmentRate')), hourlyRateCents: centsFromInput(form.get('hourlyCost')), monthlySalaryCents: salaryMode === 'monthly' ? centsFromInput(form.get('grossSalary')) : 0, status: String(form.get('status')), notes: String(form.get('notes')) };
...
function ExpenseForm({ item, workspace, busy, close, act }: { item?: Expense; workspace: Workspace; busy: boolean; close: () => void; act: ActionRunner }) {
  ...
  return <Modal title={item ? 'Modifier la dépense' : 'Nouvelle dépense'} description="Le montant net et la TVA sont saisis séparément : aucune ventilation n’est estimée." onClose={close}><form onSubmit={submitForm(async (form) => {
    const data = { projectId: String(form.get('projectId')), date: String(form.get('date')), supplier: String(form.get('supplier')), category: String(form.get('category')), reference: String(form.get('reference')), currency: 'CHF', netCents, vatCents, totalCents: netCents + vatCents, reimbursable: form.get('reimbursable') === 'yes', note: String(form.get('note')) };
  ...
  })}><div className="form-grid"><Field label={terminology.singularTitle} required wide><select name="projectId" defaultValue={item?.projectId} required><option value="">Choisir un {terminology.singular}</option>{workspace.projects.map((project) => <option value={project.id} key={project.id}>{project.name}</option>)}</select></Field><Field label="Date" required><input name="date" type="date" defaultValue={item?.date || todayIso()} required /></Field><Field label="Fournisseur" required><input name="supplier" defaultValue={item?.supplier} required autoFocus /></Field><Field label="Catégorie" required><select name="category" defaultValue={item?.category} required><option value="">Choisir une catégorie</option>{workspace.settings?.work.costCategories.map((category) => <option value={category} key={category}>{category}</option>)}</select></Field><Field label="Référence"><input name="reference" defaultValue="" /></Field><Field label="Remboursable ?" required><select name="reimbursable" defaultValue={item ? item.reimbursable ? 'yes' : 'no' : ''} required><option value="">Choisir</option><option value="yes">Oui, à rembourser</option><option value="no">Non, charge de l’entreprise</option></select></Field><Field label="Montant net (CHF)" required><input type="number" min="0" step="0.01" value={netCents ? netCents / 100 : ''} onChange={(event) => setNetCents(Math.round((event.target.valueAsNumber || 0) * 100))} required /></Field><Field label="Montant TVA (CHF)" required><input type="number" min="0" step="0.01" value={vatCents ? vatCents / 100 : ''} onChange={(event) => setVatCents(Math.round((event.target.valueAsNumber || 0) * 100))} required /></Field><Field label="Total calculé"><output className="field-output">{formatMoney(netCents + vatCents)}</output></Field><Field label="Note" wide><textarea name="note" rows={3} defaultValue={item?.note} /></Field></div><FormActions onCancel={close} busy={busy} /></form></Modal>;
}
...
function PayslipPrintSheet({ payslip, workspace, onClose }: { payslip: Payslip; workspace: Workspace; onClose: () => void }) {
  ...
  const printedPayslip = frozen ? { ...payslip, period: frozen.period, notes: frozen.notes, lines: frozen.items } : payslip;
  ...
  return <div className="print-preview"><div className="print-preview__toolbar"><strong>Aperçu de la fiche détaillée</strong><span>{error || (contributions === null ? 'Chargement des valeurs figées…' : frozen ? `Document figé le ${formatDateTime(frozen.capturedAt)}` : 'Bases, taux, plafonds et sources correspondent au calcul appliqué.')}</span><Button variant="secondary" disabled={contributions === null || Boolean(error)} onClick={() => window.print()}><Printer size={16} /> Imprimer</Button><Button variant="ghost" size="icon" onClick={onClose}><X size={18} /></Button></div>{error ? <ErrorPanel message={error} /> : null}<article className="print-sheet print-payslip"><PrintHeader settings={settings} title="FICHE DE SALAIRE" number={printedPayslip.period} /><section className="print-recipient"><span>COLLABORATEUR</span><strong>{employee?.name || '—'}</strong><p>{employee?.address || '—'}<br />{employee?.avsNumber || ''}</p></section><table className="print-table print-payroll-table"><thead><tr><th>Élément</th><th>Part / type</th><th>Base</th><th>Calcul figé</th><th>Montant</th></tr></thead><tbody>{printedPayslip.lines.map((line) => { const snapshot = snapshots.get(line.id); return <tr key={line.id}><td><strong>{line.label}</strong>{snapshot ? <small>{snapshot.source}<br />Effet {formatDate(snapshot.effectiveFrom)}{snapshot.effectiveTo ? ` → ${formatDate(snapshot.effectiveTo)}` : ''}</small> : <small>Saisie manuelle</small>}</td><td>{snapshot ? snapshot.side === 'employee' ? 'Part employé' : 'Part employeur' : line.kind === 'earning' ? 'Gain' : line.kind === 'deduction' ? 'Retenue hors moteur' : 'Charge hors moteur'}</td><td>{snapshot ? <>{formatMoney(snapshot.basisCents)}{snapshot.annualCeilingCents ? <small>Cumul avant période {formatMoney(snapshot.yearToDateBasisCents)}<br />Plafond annuel {formatMoney(snapshot.annualCeilingCents)}</small> : null}</> : '—'}</td><td>{snapshot ? snapshot.calculationKind === 'rate' ? `${((snapshot.rateBp ?? 0) / 100).toLocaleString('fr-CH')} %` : `Fixe ${formatMoney(snapshot.fixedAmountCents)}` : 'Montant saisi'}</td><td>{formatMoney(line.amountCents)}</td></tr>; })}</tbody></table><div className="print-totals"><div><span>Salaire brut</span><strong>{formatMoney(totals.earnings)}</strong></div><div><span>Retenues employé</span><strong>{formatMoney(totals.deductions)}</strong></div><div><span>Charges employeur</span><strong>{formatMoney(totals.employer)}</strong></div><div className="print-totals__grand"><span>Salaire net</span><strong>{formatMoney(totals.net)}</strong></div></div><footer className="print-footer"><p>{printedPayslip.notes}</p><p>Les cotisations calculées ci-dessus sont des valeurs figées lors de leur application à cette fiche.</p></footer></article></div>;
}

function PrintHeader({ settings, title, number }: { settings: AppSettings; title: string; number: string }) {
  const org = settings.organization;
  return <header className="print-header"><div><div className="print-brand">{org.logoPath ? <img src={convertFileSrc(org.logoPath)} alt="" /> : <HardHat size={24} />}<span>HelviChantier</span></div><strong>{org.legalName}</strong><p>{org.address.street} {org.address.buildingNumber}<br />{org.address.postalCode} {org.address.city}{org.uidNumber ? <><br />IDE {org.uidNumber}</> : null}{org.vatRegistered && org.vatNumber ? <><br />N° TVA {org.vatNumber}</> : null}</p></div><div><h1>{title}</h1><strong>{number || '—'}</strong></div></header>;
}
