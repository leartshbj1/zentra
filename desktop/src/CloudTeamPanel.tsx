...
export function CloudTeamPanel({settings}: {settings?:AppSettings|null}) {
  ...
  return <section className="cloud-team" aria-label={t('Équipe et invitations')}>
    ...
    {team?.canManage && <>
      {settings && <div className="cloud-team__profile"><div><strong>{t('Coordonnées de l’entreprise')}</strong><p>{t('Partagez le nom, les coordonnées et l’activité pour éviter de les ressaisir sur un nouvel appareil.')}</p></div><Button variant="secondary" disabled={busy} onClick={()=>void run(async()=>{await desktopApi.publishCloudCompany(settings);setNotice('Les coordonnées sont prêtes pour les appareils invités.');})}>{t(team.profile?'Actualiser les coordonnées partagées':'Partager les coordonnées')}</Button></div>}
      {team.seats.reserved>0 && <p>{t('{count} invitation(s) en attente réservent une place.',{count:team.seats.reserved})}</p>}
      <form onSubmit={event=>{event.preventDefault();void run(async()=>{if(settings)await desktopApi.publishCloudCompany(settings);const result=await desktopApi.inviteCloudMember(email.trim(),role);setLink(result.invitation.url);setNotice('Invitation créée. Transmettez le lien à cette personne.');});}}>
        ...
        <Button type="submit" disabled={busy||full}>{t('Créer le lien d’invitation')}</Button>
      </form>
