...
export function CloudTeamPanel({settings}: {settings?:AppSettings|null}) {
  ...
  return <section className="cloud-team" aria-label={t('Équipe et invitations')}>
    ...
    {team?.canManage && <>
      ...
      {inviting && <form data-company-receive-safe="true" onSubmit={event=>{event.preventDefault();void run(async()=>{if(!team.continuous)await shareCompany();setStage('Création du lien…');const result=await desktopApi.inviteCloudMember(email.trim(),role);setLink(result.invitation.url);setNotice('Invitation créée. Transmettez le lien à cette personne.');}).finally(()=>setStage(''));}}>
