...
export function PurchasesScreen({
  ...
  return <div className="stack-layout purchases-screen">
    ...
    {!supplierAccountingReady && (summary.pendingCount > 0 || summary.draftCount > 0) ? <div className="report-callout is-warning"><WalletCards size={20} /><div><strong>Comptabilité à configurer</strong><p>Complétez les comptes pour valider vos factures. Vos brouillons sont conservés.</p></div><Button variant="secondary" onClick={onOpenAccounting}>Configurer</Button></div> : null}

    <section className="panel purchase-panel">
      <SectionHeading
        title={tab === 'suppliers' ? 'Fournisseurs' : 'Factures fournisseurs'}
        action={<Button disabled={busy} onClick={tab === 'suppliers' || invoiceBlockReason ? onCreateSupplier : onCreateSupplierInvoice}><Plus size={16} /> {tab === 'suppliers' || invoiceBlockReason ? 'Ajouter un fournisseur' : 'Ajouter une facture'}</Button>}
      />
      ...
      {tab === 'suppliers' ? suppliers.length ? <div className="supplier-list" role="list">
        ...
        : supplierInvoices.length || legacyExpenses.length ? <div className="table-panel purchase-table"><table><thead><tr><th>Document</th><th>Fournisseur</th><th>{terminology.singularTitle}</th><th>Échéance / paiement</th><th>Total / solde</th><th>Statut</th><th aria-label="Actions" /></tr></thead><tbody>
          {supplierInvoices.map((invoice) => {
            ...
            return <tr key={`supplier-${invoice.id}`}><td>{formatDate(invoice.documentDate)}<small>{invoice.reference || 'Référence requise avant validation'}</small></td><td><strong>{invoice.supplierName}</strong><small>Facture fournisseur · {invoice.lines.length} ligne{invoice.lines.length > 1 ? 's' : ''} · {invoice.attachments.length} justificatif{invoice.attachments.length > 1 ? 's' : ''}</small></td><td>{project?.name || `Aucun ${terminology.singular}`}</td><td><strong className={overdue ? 'is-negative' : ''}>{formatDate(invoice.dueDate)}</strong>{invoice.paidCents > 0 ? <small>Payé {formatMoney(invoice.paidCents)}</small> : null}</td><td><strong>{formatMoney(invoice.totalCents)}</strong>{invoice.documentStatus === 'validated' ? <small>Solde {formatMoney(invoice.balanceCents)}</small> : <small>TVA {formatMoney(invoice.vatCents)}</small>}</td><td><StatusBadge status={overdue ? 'expired' : invoice.documentStatus === 'draft' ? 'draft' : invoice.paymentStatus === 'partial' ? 'partially_paid' : invoice.paymentStatus || 'draft'} label={overdue ? 'Échu' : undefined} /></td><td><div className="row-actions">{invoice.documentStatus === 'draft' ? <><Button variant="secondary" size="small" disabled={busy} title={validationReason || 'Valider et comptabiliser'} onClick={() => !invoice.reference.trim() ? onEditSupplierInvoice(invoice) : !supplierAccountingReady ? onOpenAccounting() : onValidateSupplierInvoice(invoice)}><FileCheck2 size={14} /> {!invoice.reference.trim() ? 'Compléter' : !supplierAccountingReady ? 'Configurer' : 'Valider'}</Button><Button variant="ghost" size="icon" onClick={() => onEditSupplierInvoice(invoice)} title="Modifier le brouillon et ses justificatifs" aria-label={`Modifier la facture ${invoice.reference || invoice.supplierName}`}><Pencil size={15} /></Button><Button variant="ghost" size="icon" onClick={() => onDeleteSupplierInvoiceDraft(invoice)} title="Supprimer le brouillon" aria-label={`Supprimer la facture ${invoice.reference || invoice.supplierName}`}><Trash2 size={15} /></Button></> : <><Button variant="ghost" size="small" onClick={() => onOpenSupplierInvoice(invoice)}><Eye size={14} /> Consulter</Button>{invoice.paymentStatus !== 'paid' ? <Button variant="secondary" size="small" disabled={busy} title={supplierPaymentReady ? 'Enregistrer un règlement local' : 'Choisissez le compte bancaire pour enregistrer ce paiement'} onClick={() => supplierPaymentReady ? onRecordSupplierPayment(invoice) : onOpenAccounting()}><Banknote size={14} /> {supplierPaymentReady ? 'Paiement' : 'Configurer'}</Button> : null}</>}</div></td></tr>;
          })}
