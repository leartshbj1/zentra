...
import { ArrowUpRight, ChevronRight, Clock3, FileText, FolderKanban, Receipt } from 'lucide-react';
import type { Project, Workspace } from './types';
import { salesTotalsByCurrency } from './salesFinancials';
import { formatMoney, formatMinutes } from './utils';
...
export function MobileDashboard({ workspace, onNavigate, onOpenProject, setup }: {
  ...
  const minutes = workspace.timeEntries.reduce((sum, item) => sum + item.minutes, 0);
  const links = [
  ...
  return <div className="mobile-home">
    ...
    <button type="button" className="mobile-home__time" onClick={() => onNavigate('time')}><Clock3 size={19}/><span>{t('Temps saisi')}</span><strong>{minutes ? formatMinutes(minutes) : '—'}</strong><ChevronRight size={17}/></button>
