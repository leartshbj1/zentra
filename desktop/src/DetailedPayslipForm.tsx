...
export function DetailedPayslipForm({ item, workspace, busy, close, act }: { item?: Payslip; workspace: Workspace; busy: boolean; close: () => void; act: ActionRunner }) {
  ...
  const totals = payslipTotals({ id: item?.id ?? '', employeeId, period, status: item?.status ?? 'incomplete', lines, paymentDate, notes: item?.notes ?? '', createdAt: item?.createdAt ?? '' });
  ...
  function selectEmployee(id: string) {
    ...
    const template = workspace.employeePayrollTemplates.find((candidate) => candidate.employeeId === id);
    if (!template) return;
    const recurring = template.recurringEarnings.length
      ? template.recurringEarnings
      : template.baseSalaryCents > 0
        ? [{ label: template.salaryMode === 'monthly' ? 'Salaire mensuel' : 'Salaire de base', kind: 'earning' as const, amountCents: template.baseSalaryCents }]
        : [];
    setLines(recurring.map((line) => ({ id: createId(), label: line.label, kind: 'earning', amountCents: line.amountCents })));
    setCalculation(null);
  }
  ...
  return <Modal title={item ? 'Modifier la fiche détaillée' : 'Nouvelle fiche de salaire'} description="Gains, bases, taux, plafonds et parts sont visibles. Aucune retenue n’est estimée." onClose={close} wide><form onSubmit={submitForm(async (form) => {
    ...
    const data = { employeeId, period, status, grossCents: totals.earnings, deductionsCents: totals.deductions, netCents: totals.net, employerCostsCents: totals.employer, paymentDate, notes: String(form.get('notes')) };
