...
import { normalizeComposition, documentFontCss, documentFontChoices, type DocumentComposition } from './documentComposition';
  ...
  return <DocumentWorkbench expanded={expanded} busy={busy || exporting} onClose={() => setExpanded(false)}><section className={`design-studio settings-card--wide${writing && panel === 'text' ? ' design-studio--writing' : ''}`} data-panel={panel} data-mobile-view={mobileView} aria-label="Personnalisation des documents" onKeyDown={event => {
    ...
    <div className="design-studio__body">
      <div ref={toolsElement} className="design-studio__tools">
        ...
        <label>Police du document<select aria-label="Police du document" value={design.fontFamily} disabled={busy} onChange={e => compose({ fontFamily: e.target.value as DocumentComposition['fontFamily'] })}>{documentFontChoices.map(font => <option key={font.value} value={font.value}>{font.name} · {font.description}</option>)}</select><span className="design-studio__font-sample" style={{ fontFamily: documentFontCss[design.fontFamily] }} aria-hidden="true">Aa · Votre entreprise, votre style.</span>{['inter', 'literata'].includes(design.fontFamily) && <small>Cette police fonctionne hors ligne et reste intégrée dans vos PDF.</small>}</label>
        ...
        <label>Police du titre<select aria-label="Police du titre" value={design.titleFontFamily ?? ''} disabled={busy} onChange={e => compose({ titleFontFamily: e.target.value ? e.target.value as DocumentComposition['titleFontFamily'] : undefined })}><option value="">Suivre la police du document</option>{documentFontChoices.map(font => <option key={font.value} value={font.value}>{font.name}</option>)}</select><small>Le texte courant et les passages mis en forme gardent leur propre police.</small></label>
