...
export function RemindersScreen({ workspace }: { workspace: Workspace }) {
  ...
  async function record(reminder: Reminder, action: 'printed' | 'sent_manually') {
    if (action === 'sent_manually') {
      ...
      if (note === null) return;
      await run(async () => { await desktopApi.recordReminderAction(reminder.id, action, note); await load(); }, 'L’envoi manuel a été tracé localement; aucun message n’a été envoyé par HelviChantier.');
      return;
    }
  ...
  return <div className="stack-layout reminders-screen">
    ...
    <div className="info-strip"><ShieldCheck size={17} /><span>HelviChantier ne transmet aucun e-mail : les impressions et envois manuels sont uniquement tracés dans l’historique local.</span></div>
