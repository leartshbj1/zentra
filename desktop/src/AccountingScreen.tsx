...
export function AccountingScreen({ workspace, onWorkspaceChange, focusEntry, onFocusHandled }: { workspace: Workspace; onWorkspaceChange: (workspace: Workspace) => void; focusEntry: AccountingEntryFocus | null; onFocusHandled: () => void }) {
  ...
  async function openVatJournal(entryId:string) {
  ...
  return <div className="stack-layout accounting-screen">
    ...
    {tab === 'vat' ? <VatCenter filter={filter} workspace={workspace} onAccountingChanged={reloadAll} onOpenJournal={(id)=>void openVatJournal(id)} /> : null}
...
function JournalTable({ report, onReverse, disabled }: { report: JournalReport; onReverse: (entry: JournalEntry) => void; disabled: boolean }) {
  return <div className="journal-entry-list">{report.entries.map((entry) => { const alreadyReversed = entry.hasReversal; const canReverse = !alreadyReversed && entry.reversalAction !== 'blocked_expense' && entry.reversalAction !== 'blocked_refund' && !(entry.sourceType==='expense' && !entry.reversalAction); return <article key={entry.id} data-journal-entry-id={entry.id} tabIndex={-1}><header><div><strong>{entry.number}</strong><span>{formatDate(entry.entryDate)}</span></div><p>{entry.description}</p><small>{entry.reversalOf ? 'Écriture d’extourne' : entry.sourceType === 'manual' ? 'Saisie manuelle' : entry.sourceType === 'expense_refund' ? 'Remboursement de dépense · correction depuis l’achat' : `Origine : ${entry.sourceType} · toute extourne automatique restera signalée jusqu’à correction métier`}</small>{canReverse ? <Button variant="ghost" size="small" disabled={disabled} onClick={() => onReverse(entry)}><RotateCcw size={14} /> {entry.reversalAction==='restore_expense'?'Rétablir la dépense':'Extourner'}</Button> : <span className="locked-label"><CheckCircle2 size={13} /> {alreadyReversed?'Extournée':entry.reversalAction==='blocked_refund'?'Remboursement lié à la dépense':'Dépense liée au paiement'}</span>}</header><JournalLineTable lines={report.lines.filter((line) => line.journalEntryId === entry.id)} /></article>; })}</div>;
}
