...
function JournalTable({ report, onReverse, disabled }: { report: JournalReport; onReverse: (entry: JournalEntry) => void; disabled: boolean }) {
  return <div className="journal-entry-list">{report.entries.map((entry) => { const alreadyReversed = entry.hasReversal; const canReverse = !alreadyReversed && entry.reversalAction !== 'blocked_expense' && !(entry.sourceType==='expense' && !entry.reversalAction); return <article key={entry.id} data-journal-entry-id={entry.id} tabIndex={-1}><header><div><strong>{entry.number}</strong><span>{formatDate(entry.entryDate)}</span></div><p>{entry.description}</p><small>{entry.reversalOf ? 'Écriture d’extourne' : entry.sourceType === 'manual' ? 'Saisie manuelle' : `Origine : ${entry.sourceType} · toute extourne automatique restera signalée jusqu’à correction métier`}</small>{canReverse ? <Button variant="ghost" size="small" disabled={disabled} onClick={() => onReverse(entry)}><RotateCcw size={14} /> {entry.reversalAction==='restore_expense'?'Rétablir la dépense':'Extourner'}</Button> : <span className="locked-label"><CheckCircle2 size={13} /> {alreadyReversed?'Extournée':'Dépense liée au paiement'}</span>}</header><JournalLineTable lines={report.lines.filter((line) => line.journalEntryId === entry.id)} /></article>; })}</div>;
}
