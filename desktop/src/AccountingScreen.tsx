...
type Tab = 'journal' | 'ledger' | 'trial' | 'balance' | 'income' | 'closing' | 'accounts' | 'periods';
...
export function AccountingScreen({ workspace }: { workspace: Workspace }) {
  ...
  const tabs: Array<[Tab, string, React.ReactNode]> = [
    ['journal', 'Journal', <BookOpen size={16} />], ['ledger', 'Grand livre', <ListChecks size={16} />], ['trial', 'Balance', <Scale size={16} />], ['balance', 'Bilan', <Landmark size={16} />], ['income', 'Résultat', <CheckCircle2 size={16} />], ['closing', 'Dossier de clôture', <FileCheck2 size={16} />], ['accounts', 'Plan & liaisons', <ShieldCheck size={16} />], ['periods', 'Exercices', <LockKeyhole size={16} />],
  ];
