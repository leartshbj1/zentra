...
export function VatCenter({
  ...
  return <div className="vat-center stack-layout" aria-busy={busy}>
    ...
    <section className="panel vat-navigation"><div className="tab-strip" role="tablist" aria-label="Sections de l’assistant TVA">{tabItems.map(([id, label, icon]) => <button key={id} type="button" role="tab" aria-selected={tab === id} aria-controls={`vat-panel-${id}`} className={tab === id ? 'is-active' : ''} onClick={() => setTab(id)}>{icon}{label}</button>)}</div><Button size="small" variant="secondary" disabled={busy} onClick={() => void load(true)}>Actualiser</Button></section>
