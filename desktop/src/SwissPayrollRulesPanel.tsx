...
const sources = {
  ...
  accidentPremiums: 'https://www.seco.admin.ch/fr/annonce-des-rapports',
};
...
const SWISS_RULES_REFERENCE_DATE = '2026-06-30';
...
export function SwissPayrollRulesPanel({ settings }: { settings: AppSettings }) {
  ...
  useEffect(() => {
    let alive = true;
    void desktopApi.listPayrollContributionDefinitions(SWISS_RULES_REFERENCE_DATE)
  ...
  const rules = useMemo<Rule[]>(() => {
    ...
    const aanpCoverage = settings.payroll.aanpEmployerCoverage;
    const aanpEmployerEvidenceComplete = Boolean(
      aanpCoverage?.enabled &&
      aanpCoverage.reference.trim() &&
      aanpCoverage.effectiveFrom,
    );
    return [
      ...
      { id: 'laa', title: 'Assurance-accidents LAA', value: 'Gain assuré max. CHF 148’200/an', applicability: 'AAP pour tous les salariés; AANP dès 8 h/semaine chez le même employeur. AAP à charge employeur; AANP à charge salarié sauf convention plus favorable.', configuration: aanpCoverage?.enabled ? aanpEmployerEvidenceComplete ? `Prise en charge employeur documentée par « ${aanpCoverage.reference.trim()} » dès le ${aanpCoverage.effectiveFrom}. La source de la définition AANP doit correspondre exactement.` : 'La prise en charge employeur est activée mais sa référence et sa période ne sont pas complètes.' : 'Saisir l’assureur, la classe de risque et les primes réelles. Sans convention plus favorable structurée, AANP reste côté salarié.', source: sources.accidentPremiums, status: configured('aap', settings.payroll.accidentInsurer) && configured('aanp', settings.payroll.accidentInsurer) && (!aanpCoverage?.enabled || aanpEmployerEvidenceComplete) ? 'configured' : 'missing' },
