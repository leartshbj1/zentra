...
export function SupplierInbox({inbox,workspace,readOnly,onOpen,onCreateSupplier,embedded=false}:{inbox:ReturnType<typeof useSupplierInbox>;workspace:Workspace;readOnly:boolean;onOpen:(id:string)=>void;onCreateSupplier:CreateSupplier;embedded?:boolean}){
  useAppLanguage();
  const [open,setOpen]=useState(false),[selected,setSelected]=useState<MailInvoice|null>(null),[notice,setNotice]=useState('');
  const [filter,setFilter]=useState<'pending'|'imported'>('pending'),[query,setQuery]=useState(''),[savedId,setSavedId]=useState('');
  useEffect(()=>{setSelected(null);setOpen(false);setNotice('');setQuery('');setFilter('pending');setSavedId('');},[inbox.state?.organizationId]);
  const pending=pendingMailInvoices(inbox.state);
  const ready=inbox.state?.linked;
  const visible=embedded||open;
  const items=filterMailInvoices(inbox.state?.items||[],filter,query);
  return <section className={`supplier-inbox${embedded?' supplier-inbox--embedded':''}`} aria-label={t('Boîte de réception fournisseurs')}>
    <header><span className="supplier-inbox__icon"><Inbox size={22} aria-hidden="true"/></span><div><h2>{t(embedded?'Factures reçues':'Boîte de réception fournisseurs')}{pending.length>0&&<span className="supplier-inbox__count">{pending.length}</span>}</h2><p>{t(!inbox.state?'Chargement…':ready?'Les factures de votre boîte mail, réunies pour toute l’équipe.':'Reliez votre boîte mail pour retrouver vos factures ici.')}</p></div>{inbox.state&&(!embedded||!ready)&&<Button variant="secondary" onClick={()=>ready?setOpen(!open):void invoke('open_supplier_inbox_settings')}>{t(ready?(open?'Réduire':'Voir les factures'):'Relier Support')}<ArrowRight size={16}/></Button>}</header>
    ...
    {notice&&<div className="supplier-inbox__notice" role="status">{t(notice)}{savedId&&embedded&&<Button variant="ghost" onClick={()=>onOpen(savedId)}>{t('Ouvrir la facture')}</Button>}</div>}
    {visible&&ready&&<>
      {inbox.state?.prepareEnabled&&pending.length>1&&<div className="supplier-inbox__batch"><div><p>{t('Les montants et les fournisseurs sont vérifiés ensemble.')}</p></div><Button disabled={inbox.busy||readOnly} onClick={()=>void inbox.prepareAll(workspace)}>{inbox.busy&&inbox.batch?`${inbox.batch.done} / ${inbox.batch.total}`:t('Vérifier toutes les factures')}</Button></div>}
      {inbox.batch&&<details className="supplier-inbox__batch-result" open><summary>{inbox.batch.done} / {inbox.batch.total} factures traitées</summary><ul>{inbox.batch.results.map(r=><li key={r.id}><strong>{r.label}</strong><span>{r.outcome}</span></li>)}</ul></details>}
      <div className="supplier-inbox__toolbar"><div className="supplier-inbox__filters" role="group" aria-label={t('Afficher les factures')}><button type="button" aria-pressed={filter==='pending'} onClick={()=>setFilter('pending')}>{t('À vérifier')}<span>{pending.length}</span></button><button type="button" aria-pressed={filter==='imported'} onClick={()=>setFilter('imported')}>{t('Enregistrées')}</button></div><input type="search" aria-label={t('Rechercher un fournisseur ou une référence')} placeholder={t('Fournisseur ou référence')} value={query} onChange={event=>setQuery(event.target.value)}/></div>
      <div className="supplier-inbox__list">{items.map(item=><article key={item.id}><FileText size={20} aria-hidden="true"/><div><strong>{item.extraction.supplierName||item.sender||item.fileName}</strong><span>{item.extraction.reference||item.fileName}</span><small>{t(item.state==='imported'?(item.automatic?'Comptabilisée par Automation':'Enregistrée dans Gestion'):item.otherDevice?'Import en cours sur un autre appareil':'À vérifier')}</small></div><b>{item.extraction.totalCents!==null?new Intl.NumberFormat(getAppLocale(),{style:'currency',currency:item.extraction.currency||'CHF'}).format(item.extraction.totalCents/100):'—'}</b><Button variant="secondary" aria-label={`${t(item.state==='imported'?'Ouvrir':'Vérifier')} · ${item.extraction.reference||item.fileName}`} disabled={inbox.busy||item.otherDevice} onClick={()=>item.state==='imported'?onOpen(item.invoiceId||item.id):setSelected(item)}>{t(item.state==='imported'?'Ouvrir':'Vérifier')}</Button></article>)}{items.length===0&&!inbox.error&&<div className="supplier-inbox__empty"><Check size={24} aria-hidden="true"/><strong>{t(query?'Aucune facture ne correspond à cette recherche.':filter==='pending'?'Tout est à jour.':'Aucune facture enregistrée pour le moment.')}</strong>{!query&&filter==='pending'&&<p>{t('Votre prochaine facture apparaîtra ici automatiquement.')}</p>}</div>}</div>
    </>}
    {visible&&ready&&<Button variant="ghost" onClick={()=>void invoke('open_supplier_inbox_settings')}><Link2 size={15}/>{t('Réglages de la boîte mail')}</Button>}
    {selected&&<ReviewMailInvoice habits={inbox.state?.habits||[]} automationActive={!!inbox.state?.prepareEnabled} key={selected.id} item={selected} workspace={workspace} busy={inbox.busy} readOnly={readOnly} onClose={()=>setSelected(null)} onCreateSupplier={onCreateSupplier} onIgnore={async()=>{await inboxRequest({action:'ignore',id:selected.id});setSelected(null);await inbox.refresh();}} onSave={async (draft,confirm)=>{const result=await inbox.importInvoice(selected,draft,confirm);setSelected(null);setSavedId(result.id);setNotice(result.posted?'Facture comptabilisée. Le justificatif est joint et les montants sont mis à jour.':'Brouillon enregistré avec son justificatif.');if(!embedded)onOpen(result.id);}}/>}
  </section>;
}
function ReviewMailInvoice({habits,automationActive,item,workspace,busy,readOnly,onClose,onCreateSupplier,onIgnore,onSave}:{habits:SupplierHabit[];automationActive:boolean;item:MailInvoice;workspace:Workspace;busy:boolean;readOnly:boolean;onClose:()=>void;onCreateSupplier:CreateSupplier;onIgnore:()=>Promise<void>;onSave:(draft:import('./supplierInbox').InboxDraft,confirm:boolean)=>Promise<void>}){
