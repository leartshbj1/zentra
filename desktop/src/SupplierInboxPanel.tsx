...
export function SupplierInbox({inbox,workspace,readOnly,onOpen,onCreateSupplier,embedded=false}:{inbox:ReturnType<typeof useSupplierInbox>;workspace:Workspace;readOnly:boolean;onOpen:(id:string)=>void;onCreateSupplier:CreateSupplier;embedded?:boolean}){
  ...
  return <section className={`supplier-inbox${embedded?' supplier-inbox--embedded':''}`} aria-label={t('Boîte de réception fournisseurs')}>
    ...
    {selected&&<ReviewMailInvoice key={selected.id} item={selected} workspace={workspace} busy={inbox.busy} readOnly={readOnly} onClose={()=>setSelected(null)} onCreateSupplier={onCreateSupplier} onIgnore={async()=>{await inboxRequest({action:'ignore',id:selected.id});setSelected(null);await inbox.refresh();}} onSave={async (draft,confirm)=>{const result=await inbox.importInvoice(selected,draft,confirm);setSelected(null);setSavedId(result.id);setNotice(result.posted?'Facture comptabilisée. Le justificatif est joint et les montants sont mis à jour.':'Brouillon enregistré avec son justificatif.');if(!embedded)onOpen(result.id);}}/>}
  </section>;
}
function ReviewMailInvoice({item,workspace,busy,readOnly,onClose,onCreateSupplier,onIgnore,onSave}:{item:MailInvoice;workspace:Workspace;busy:boolean;readOnly:boolean;onClose:()=>void;onCreateSupplier:CreateSupplier;onIgnore:()=>Promise<void>;onSave:(draft:import('./supplierInbox').InboxDraft,confirm:boolean)=>Promise<void>}){
  ...
  return <Modal title={t('Votre facture est préparée')} onClose={onClose} dismissible={!working} wide className="supplier-inbox-review"><form onInvalid={()=>setEditing(true)} onSubmit={event=>{event.preventDefault();void confirmInvoice(true);}}>
    ...
    <Field label={t('Fournisseur')}><select required value={supplier} onChange={event=>setSupplier(event.target.value)}><option value="">{t('Choisir un fournisseur')}</option>{workspace.suppliers.filter(s=>!s.archivedAt).map(s=><option key={s.id} value={s.id}>{s.name}</option>)}</select></Field><Button type="button" variant="ghost" onClick={()=>setAddingSupplier(!addingSupplier)}>{t(addingSupplier?'Annuler':'Ajouter un fournisseur')}</Button>
