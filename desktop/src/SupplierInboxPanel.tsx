...
import { SupplierInboxBatchResult } from './SupplierInboxBatchResult';
...
export function SupplierInbox({inbox,workspace,readOnly,onOpen,onCreateSupplier,embedded=false}:{inbox:ReturnType<typeof useSupplierInbox>;workspace:Workspace;readOnly:boolean;onOpen:(id:string)=>void;onCreateSupplier:CreateSupplier;embedded?:boolean}){
  ...
  return <section className={`supplier-inbox${embedded?' supplier-inbox--embedded':''}`} aria-label={t('Boîte de réception fournisseurs')}>
    ...
    {visible&&ready&&<>
      {inbox.state?.prepareEnabled&&pending.length>1&&<div className="supplier-inbox__batch"><p>{t('Fournisseurs et factures, préparés ensemble.')}</p><Button disabled={inbox.busy||readOnly} onClick={()=>void inbox.prepareAll(workspace)}>{t(inbox.busy?'Vérification en cours…':'Vérifier toutes les factures')}</Button></div>}
      {inbox.batch&&<SupplierInboxBatchResult batch={inbox.batch} busy={inbox.busy} onOpen={onOpen} onReview={id=>{const item=inbox.state?.items.find(row=>row.id===id);if(item)setSelected(item);}}/>}
