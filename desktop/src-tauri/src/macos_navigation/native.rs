...
use objc2_app_kit::{
    NSControlSize, NSLayoutConstraint, NSSegmentDistribution, NSSegmentStyle, NSSegmentedControl,
    NSView, NSWindow,
};
    ...
    impl NavigationAction {
        ...
        fn navigate(&self, control: &NSSegmentedControl) {
            ...
            if let Some(id) = usize::try_from(index).ok().and_then(|index| DESTINATIONS.get(index)) {
                if !(self.ivars().channel.borrow())(id) {
...
impl NavigationAction {
    fn new(mtm: MainThreadMarker, channel: Box<dyn Fn(&str) -> bool>) -> Retained<Self> {
...
pub fn configure(
    ...
    let window_id = native_window as *const NSWindow as usize;
    ...
    DOCK.with_borrow_mut(|slot| {
        if slot
            .as_ref()
            .is_some_and(|dock| dock.window_id != window_id)
        {
            *slot = None;
        }
        if slot.is_none() {
