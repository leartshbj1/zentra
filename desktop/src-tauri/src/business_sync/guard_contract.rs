//! Trusted native BEFORE guards and their actual SQLite read dependencies.
...
fn catalog(connection: &Connection) -> Value {
    ...
    let parser = Regex::new(r"(?is)^CREATE TRIGGER(?: IF NOT EXISTS)? (\w+)\s+BEFORE (INSERT|UPDATE|DELETE)(?: OF ([\w,\s]+))? ON (\w+)\s+(?:WHEN\s+(.*?)\s+)?BEGIN\s+SELECT RAISE\(ABORT\s*,\s*'((?:[^']|'')*)'\);\s*END\s*;?$").unwrap();
    ...
    for (name, table, sql) in rows {
        if !policy.tables.contains_key(&table)
            || !sql
                .split_whitespace()
                .any(|w| w.eq_ignore_ascii_case("BEFORE"))
        {
            continue;
        }
        let sql = sql.replace("\r\n", "\n");
    ...
    let views = views
        ...
        .collect::<BTreeMap<_, _>>();
    json!({"version":1,"native_schema_version":crate::schema::SCHEMA_VERSION,"data_schema_version":DATA_SCHEMA_VERSION,"guards":guards,"views":views})
}
...
fn native_guard_catalog_matches_every_current_shared_before_trigger() {
    ...
    let actual = catalog(&store.connect().unwrap());
