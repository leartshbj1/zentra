  ...
  try {
    for (const width of [320, 390, 1440]) {
      ...
      try {
        ...
        const payment = page.getByRole('dialog', { name: 'Paiement fournisseur', exact: true });
        await payment.getByRole('textbox', { name: /^Montant payé/ }).fill('108,11');
        await payment.getByRole('button', { name: 'Vérifier le paiement', exact: true }).click();
        ...
        await payment.locator('.supplier-payment__optional summary').click();
        ...
        await payment.getByRole('button', { name: 'Actualiser les achats', exact: true }).click();
        await payment.getByRole('button', { name: 'Vérifier le paiement', exact: true }).click();
