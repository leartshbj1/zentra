...
try {
  for (const width of [320, 390, 768, 1024, 1440]) {
    ...
    await payments.getByRole('button', { name: 'Afficher les règlements suivants', exact: true }).click();
    ...
    await payments.getByRole('searchbox', { name: 'Rechercher un règlement', exact: true }).fill('etude');
    assert.equal(await rows.count(), 16);
    await payments.getByRole('searchbox', { name: 'Rechercher un règlement', exact: true }).fill('F-2026-030');
    assert.equal(await rows.count(), 1);
    assert.match(await rows.textContent(), /46[.,]25/);
    assert.match(await rows.textContent(), /3[.,]75/);
    await payments.getByRole('searchbox', { name: 'Rechercher un règlement', exact: true }).fill('introuvable');
    await payments.getByText('Aucun règlement ne correspond à cette recherche.', { exact: true }).waitFor();
    await payments.getByRole('searchbox', { name: 'Rechercher un règlement', exact: true }).fill('');
    await payments.getByLabel('Type de règlement', { exact: true }).selectOption('supplier_invoice_item');
    await payments.getByRole('searchbox', { name: 'Rechercher un règlement', exact: true }).fill('ACH-2026-031');
