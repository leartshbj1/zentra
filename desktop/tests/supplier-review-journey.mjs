  ...
  try {
    for (const [width, height] of [[320, 568], [390, 844], [844, 390], [1440, 900]]) {
      ...
      try {
        ...
        assert.equal(await review().locator('button[type=submit]:not(:disabled)').count(), 0);
        assert.equal((await attempts('validate')).length, 0);
        await review().locator('.form-actions').getByRole('button', { name: 'Compléter la référence', exact: true }).click();
        ...
        await review().getByRole('button', { name: 'Relire la facture', exact: true }).click();
        assert.equal((await attempts('validate')).length, 2, 'Relire ne valide pas la facture');
        assert.equal((await docs())[0].documentStatus, 'draft');
        await capture('ready');
        ...
        const payment = page.locator('.supplier-payment-modal');
