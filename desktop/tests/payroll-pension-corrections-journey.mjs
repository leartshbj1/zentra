...
try {
  for (const width of [320, 390, 1440]) {
    ...
    const guide = modal.locator('.payroll-preparation');
    ...
    const [annualChange] = await page.evaluate(() => JSON.parse(sessionStorage.getItem('qa-payroll-employee-update')));
    assert.deepEqual(Object.keys(annualChange.data).sort(), ['lppAnnualSalaryCents', 'lppAssessmentYear']);
    ...
    assert.equal(await pair.locator('[name=source]').inputValue(), 'Règlement QA pension 2026');
    assert.equal(await pair.locator('[name=from]').inputValue(), '2026-01-01');
