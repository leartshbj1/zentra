...
for(const viewport of [{width:320,height:568},{width:390,height:844},{width:844,height:390}]){
 ...
 for(const view of ['document','image','pdf']){
  ...
  // Safe-area changes trigger ResizeObserver and a React render. Start the
  // gesture only after the document has fitted its new available width.
  if(view==='document')await page.waitForFunction(()=>{const area=document.querySelector('[data-touch-document]'),paper=document.querySelector('.document-preview__paper');if(!area||!paper)return false;const css=getComputedStyle(area);const fit=Math.min(1,(area.clientWidth-parseFloat(css.paddingLeft)-parseFloat(css.paddingRight))/794);return Math.abs(new DOMMatrix(getComputedStyle(paper).transform).a-fit)<0.001;});
  ...
  await page.waitForFunction(({selector,before})=>{const paper=document.querySelector(selector);return paper&&new DOMMatrix(getComputedStyle(paper).transform).a>before*2;},{selector,before});
...
await page.evaluate(()=>window.connect());await page.getByRole('button',{name:'join',exact:true}).click();await page.getByRole('button',{name:'Recevoir et ouvrir l’entreprise',exact:true}).waitFor();assert.equal(await page.getByRole('button',{name:'Recevoir et ouvrir l’entreprise',exact:true}).isDisabled(),true,'missing company copy prevents an empty import');await page.getByText('Demandez au titulaire d’activer le partage dans Compte et équipe.',{exact:true}).waitFor();await page.getByRole('button',{name:/Fermer/}).first().click();
assert.equal(await page.getByRole('button',{name:'Créer le lien d’invitation',exact:true}).count(),0,'invitation fields stay collapsed until requested');
await page.getByRole('button',{name:'Inviter une personne',exact:true}).click();
assert.equal(await page.getByRole('button',{name:'Créer le lien d’invitation',exact:true}).isEnabled(),true,'invitation prepares the full company copy automatically');assert.equal(await page.getByRole('checkbox').count(),0,'there is only one full-company sharing mode');await page.getByLabel(/^Adresse e-mail/).fill('colleague@example.invalid');await page.getByLabel('Rôle',{exact:true}).selectOption('read_only');await page.getByRole('button',{name:'Créer le lien d’invitation',exact:true}).click();await page.getByLabel('Lien réservé à l’adresse invitée',{exact:true}).waitFor();const invitationCalls=await page.evaluate(()=>window.calls);assert.ok(invitationCalls.indexOf('company-copy')>=0&&invitationCalls.indexOf('company-copy')<invitationCalls.indexOf('invite:read_only'),'full copy prepared before inviting');
