...
await page.evaluate(()=>window.connect());await page.getByRole('button',{name:'join',exact:true}).click();await page.getByRole('button',{name:'Recevoir et ouvrir l’entreprise',exact:true}).waitFor();assert.equal(await page.getByRole('button',{name:'Recevoir et ouvrir l’entreprise',exact:true}).isDisabled(),true,'missing company copy prevents an empty import');await page.getByText('Le titulaire doit choisir « Partager l’entreprise complète » dans Paramètres → Compte et équipe. Actualisez ensuite cet écran.',{exact:true}).waitFor();await page.getByRole('button',{name:/Fermer/}).first().click();
assert.equal(await page.getByRole('button',{name:'Créer le lien d’invitation',exact:true}).isDisabled(),true,'full company sharing needs consent');await page.getByRole('checkbox').check();await page.getByLabel(/^Adresse e-mail/).fill('colleague@example.invalid');await page.getByLabel('Rôle',{exact:true}).selectOption('read_only');await page.getByRole('button',{name:'Créer le lien d’invitation',exact:true}).click();await page.getByLabel('Lien réservé à l’adresse invitée',{exact:true}).waitFor();const invitationCalls=await page.evaluate(()=>window.calls);assert.ok(invitationCalls.indexOf('company-copy')>=0&&invitationCalls.indexOf('company-copy')<invitationCalls.indexOf('invite:read_only'),'full copy prepared before inviting');
