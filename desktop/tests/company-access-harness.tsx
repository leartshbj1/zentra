...
let joined=0, resets=0, shared=0, invited=0, receiptConfirmations=0;
const duplicateReceipt={localId:'qa-local',remoteId:'qa-remote',invoiceId:'qa-invoice',invoiceNumber:'F-2026-0100',amountCents:10000,currency:'CHF',date:'2026-09-15',fingerprint:'a'.repeat(64)};
...
desktopApi.syncCompanyWorkspace=async(receive,acceptRemote,choice)=>{if(choice){if(receive||acceptRemote||JSON.stringify(choice)!==JSON.stringify(duplicateReceipt))throw new Error('Only the reviewed receipt can be confirmed');receiptConfirmations++;}return {enabled:true,revision:1,pending:false,conflict:false};};
publishCompanySync({enabled:true,revision:1,pending:false,conflict:false,lastSyncedAt:'2026-09-15T12:00:00Z'});
if(query.has('duplicate'))publishCompanySync({enabled:true,revision:1,pending:true,conflict:true,conflictReason:'La facture F-2026-0100 a reçu deux saisies à vérifier.',duplicateReceipt});
...
Object.assign(window,{companyFixture:{get joined(){return joined},get resets(){return resets},get shared(){return shared},get invited(){return invited},get receiptConfirmations(){return receiptConfirmations}}});
function Harness(){
  useMobileLayout();
  const [result,setResult]=useState<Workspace|null>(null),[closed,setClosed]=useState(false);
  if(query.has('account'))return <main style={{maxWidth:850,margin:'auto',padding:16}}><CloudAccountPanel settings={initialOnboardingSettings}/></main>;
