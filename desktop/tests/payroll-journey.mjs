...
try {
  for (const width of [320, 390, 768, 1024, 1440]) {
    ...
    await modal.locator('.payroll-problems').getByText('Voir le message détaillé', { exact: true }).click();
    ...
    await page.getByRole('button', { name: 'Finaliser la fiche', exact: true }).waitFor();
    assert.equal((await calls(page, 'save')).length, 2, 'One refusal and one acknowledged save');
    await set(page, 'recover-post');
    await page.getByRole('button', { name: 'Finaliser la fiche', exact: true }).click();
    await page.getByRole('dialog', { name: 'Finaliser la fiche', exact: true }).getByRole('button', { name: 'Enregistrer en comptabilité', exact: true }).click();
    await page.getByRole('dialog', { name: 'Enregistrement effectué', exact: true }).waitFor();
    await set(page, 'fail-reads', '0');
    await page.getByRole('button', { name: /Actualiser les données/ }).click();
    await page.getByRole('dialog', { name: 'La fiche est en comptabilité', exact: true }).waitFor();
    assert.equal((await calls(page, 'post')).length, 1);
    await page.getByRole('dialog', { name: 'La fiche est en comptabilité', exact: true }).getByText('Des comptes généraux ont été utilisés', { exact: true }).waitFor();
    await page.getByRole('button', { name: 'Le virement est fait', exact: true }).click();
    await modal.locator('input[name=paymentDate]').fill('2026-09-30');
    ...
    await page.locator('.payslip-list > article').first().getByRole('button', { name: 'Voir le PDF', exact: true }).click();
