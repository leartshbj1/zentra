    ...
    for (const width of [320, 390, 1440]) for (const kind of ['save', 'revision', 'correction', 'payment']) {
      ...
      try {
        ...
        if (kind === 'save') {
          ...
          assert.equal(await form.locator('[name=amount]').inputValue(), '');
          assert.equal(await form.locator('[name=method]').inputValue(), 'Virement bancaire');
          await form.locator('summary').filter({hasText:'Référence et note'}).click();
          await form.locator('[name=reference]').fill('VIREMENT-42');
          await page.evaluate(() => sessionStorage.setItem('qa-sales-block-reads','1'));
          await form.getByRole('button',{name:'Actualiser les factures',exact:true}).click();
          await form.getByText(/^La lecture des factures est interrompue/).waitFor();
          await page.evaluate(() => sessionStorage.removeItem('qa-sales-block-reads'));
          await form.getByRole('button',{name:'Actualiser les factures',exact:true}).click();
          await form.locator('[name=amount]').fill('1082');
          await form.getByRole('button',{name:'Vérifier le paiement',exact:true}).click();
          await form.getByText(/^Ce montant dépasse le reste à encaisser/).waitFor();
          await form.locator('[name=amount]').fill('1081');
          await form.locator('[name=date]').fill('2026-08-31');
          await form.getByRole('button',{name:'Vérifier le paiement',exact:true}).click();
          await form.getByText(/^Indiquez le jour réel de réception/).waitFor();
          ...
          await form.getByText(/^Cette date appartient à un exercice fermé/).waitFor();
          ...
          await form.getByRole('button',{name:'Modifier',exact:true}).click();
