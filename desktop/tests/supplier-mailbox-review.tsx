...
const scenario=new URLSearchParams(location.search).get('supplier');
if(scenario){workspace.suppliers=[];item.extraction={...item.extraction,kind:'supplier_invoice',kindConfidence:.99,fieldConfidence:{supplierName:scenario==='uncertain'?.4:.99},confidence:0,category:null,issues:['Vérifiez la catégorie de la dépense.']};}
Object.assign(window,{__TAURI_INTERNALS__:{invoke:async(_command:string,args:{data?:{action?:string}})=>{if(args?.data?.action==='document'){const response=await fetch('/tests/fixtures/automation-test-invoice.pdf');if(!response.ok)throw Error('Missing QA PDF');const bytes=new Uint8Array(await response.arrayBuffer());return{base64:btoa(String.fromCharCode(...bytes))};}if(args?.data?.action==='prepareSupplier'){if(scenario==='ambiguous')throw Error('Plusieurs fournisseurs portent ce nom. Choisissez celui qui figure sur la facture.');return{supplierId:'qa-auto-vendor',created:true};}return{};}}});
