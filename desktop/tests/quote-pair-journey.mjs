...
try {
  for (const width of [320, 390, 768, 1440]) {
    for (const legacy of [false, true]) {
      ...
      await page.getByText('Voir le détail du montant', { exact: true }).click();
      ...
      assert.match(await page.locator('.paired-preparation__notes').innerText(), /Conditions du solde\nDeuxième ligne conservée\./);
      ...
      await page.getByRole('button', { name: 'Vérifier la facture', exact: true }).click();
      await page.getByRole('button', { name: 'Enregistrer et voir le dossier', exact: true }).click();
      await page.getByRole('button', { name: 'Ouvrir l’acompte', exact: true }).waitFor();
      ...
      await page.waitForFunction(() => document.querySelector('.paired-invoice-editor input[name=serviceDateFrom]')?.matches(':disabled'));
