...
import { installProjectSyncRecoveryFixture } from './project-sync-recovery-fixture';
import { useProjectSyncBackground } from '../src/projectSync';
...
const syncRecovery = new URLSearchParams(location.search).has('syncRecovery');
if (syncRecovery) await installProjectSyncRecoveryFixture(() => workspace);
...
function SyncBackground({ onWorkspace }: { onWorkspace: (value: Workspace) => void }) {
  useProjectSyncBackground(onWorkspace); return null;
}
function Harness() {
  const [data, setData] = useState(() => structuredClone(workspace));
  return <main style={{ maxWidth: 1100, padding: 16, margin: '0 auto' }}>{syncRecovery && <SyncBackground onWorkspace={setData} />}<ProjectFolder project={project} workspace={data} busy={false} readOnly={new URLSearchParams(location.search).has('readonly')} onBack={() => {}} onOpenDocument={() => {}} onCreateDocument={() => {}} onWorkspaceChange={value => setData(structuredClone(value))} /></main>;
}
