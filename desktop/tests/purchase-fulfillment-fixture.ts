...
export function installPurchaseFulfillmentFixture(initial: Workspace) {
  ...
  desktopApi.saveSupplierCreditNoteDraft = async (input) => {
    ...
    const credit = { id, supplierId: input.supplierId, documentDate: input.documentDate, number: '', reference: input.reference || '', note: input.note || '', status: 'draft', currency: 'CHF', supplierName: initial.suppliers[0].name, items: lines, netCents: lines.reduce((sum, line) => sum + line.netCents, 0), vatCents: lines.reduce((sum, line) => sum + line.vatCents, 0), totalCents: lines.reduce((sum, line) => sum + line.totalCents, 0), allocations: [], allocatedCents: 0, refundedCents: 0, refunds: [], validatedAt: null, validationJournalEntryId: null, createdAt: now, updatedAt: now } as SupplierCreditNote;
