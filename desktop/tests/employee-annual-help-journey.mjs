...
const reports = [];
await mkdir('.qa/employee-annual-help', { recursive: true });
for (const [engine, browserType] of [['edge', chromium], ['webkit', webkit]]) {
  ...
  try {
    for (const [width, height] of [[320, 568], [390, 844], [844, 390], [1440, 900]]) {
      ...
      await modal.locator('input[name=smallSalaryAssessmentYear]').fill('2026');
      await modal.locator('select[name=smallSalarySector]').selectOption('ordinary');
      await modal.locator('select[name=smallSalaryEmployeeRequestedContributions]').selectOption('no');
      await modal.locator('input[name=smallSalaryDecisionDate]').fill('2025-12-31');
      await modal.locator('input[name=smallSalaryOpeningGross]').fill('0');
      await modal.locator('input[name=smallSalaryOpeningContributedBasis]').fill('0');
      await modal.locator('input[name=smallSalaryEvidenceReference]').fill('Déclaration de recette');
      await advanced.click();
      ...
      await page.screenshot({ path: `.qa/employee-annual-help/${engine}-${width}-date.png` });
      assert.equal(await modal.evaluate(el => el.scrollWidth <= el.clientWidth + 1), true);
      await modal.getByRole('button', { name: 'Compléter ce réglage plus tard', exact: true }).click();
      assert.equal(await date.isVisible(), false);
      await modal.getByRole('button', { name: 'Reprendre ce réglage maintenant', exact: true }).click();
      assert.equal(await date.inputValue(), '2025-12-31');
      if (width === 320) await modal.getByRole('button', { name: 'Compléter ce réglage plus tard', exact: true }).click();
      ...
      assert.equal(write.smallSalaryDecisionDate, width === 320 ? '' : '2026-01-12');
      assert.equal(write.smallSalaryAssessmentYear, width === 320 ? null : 2026);
      assert.equal(write.smallSalarySector, width === 320 ? null : 'ordinary');
...
await writeFile('.qa/employee-annual-help/report.json', JSON.stringify(reports, null, 2));
