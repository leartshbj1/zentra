  ...
  try {
    for (const [width, height] of [[320, 568], [390, 844], [844, 390], [1440, 900]]) {
      ...
      try {
        ...
        await page.evaluate(() => sessionStorage.setItem('qa-pair-refuse-convert', '1'));
        await page.getByRole('button', { name: 'Créer les deux factures', exact: true }).click();
        await page.getByRole('alert').filter({ hasText: 'La conversion n’a pas été enregistrée. Réessayez.' }).waitFor();
        assert.equal(await page.getByRole('textbox', { name: 'Pourcentage de l’acompte', exact: true }).inputValue(), '40');
        await page.evaluate(() => { sessionStorage.removeItem('qa-pair-refuse-convert'); sessionStorage.setItem('qa-pair-hold-convert', '1'); });
        await page.getByRole('button', { name: 'Créer les deux factures', exact: true }).click();
        assert.ok(await page.getByRole('textbox', { name: 'Pourcentage de l’acompte', exact: true }).isDisabled());
        await page.keyboard.press('Escape');
        assert.ok(await page.getByRole('dialog', { name: 'Créer la facture', exact: true }).isVisible());
        await page.evaluate(() => { sessionStorage.removeItem('qa-pair-hold-convert'); window.dispatchEvent(new Event('qa-release-pair-convert')); });
        ...
        const issue = page.getByRole('dialog', { name: 'Émettre l’acompte', exact: true });
        await issue.waitFor();
        assert.match(await issue.locator('.invoice-issue__total').innerText(), /432.40/);
        await issue.getByRole('button', { name: 'Revenir sans émettre', exact: true }).click();
        assert.equal(await page.evaluate(() => sessionStorage.getItem('qa-pair-issue-attempts')), null);
        await deposit.getByRole('button', { name: 'Émettre l’acompte', exact: true }).click();
        ...
        const correctDate = issue.getByRole('button', { name: 'Corriger la date du brouillon', exact: true });
        await correctDate.scrollIntoViewIfNeeded();
        ...
        await issue.getByRole('button', { name: 'Confirmer et émettre l’acompte', exact: true }).click();
        await issue.getByText('La période est fermée', { exact: true }).waitFor();
        await issue.getByRole('button', { name: 'Vérifier les exercices', exact: true }).click();
        ...
        await issue.getByRole('button', { name: 'Confirmer et émettre l’acompte', exact: true }).evaluate(button => { button.click(); button.click(); });
        assert.equal(await issue.locator('form button:not(:disabled)').count(), 0);
        assert.equal(await page.getByRole('button', { name: 'Fermer « Émettre l’acompte »', exact: true }).count(), 0);
        await page.keyboard.press('Escape');
        assert.ok(await issue.isVisible());
        assert.equal(await page.evaluate(() => Number(sessionStorage.getItem('qa-pair-issue-attempts'))), 3);
        ...
        async function savePayment(){ const verify=payment.getByRole('button',{name:'Vérifier le paiement',exact:true}); if(await verify.count()){if(!await payment.locator('[name=amount]').inputValue())await payment.getByRole('button',{name:/Tout le solde est reçu/}).click();await verify.click();}await payment.getByRole('button',{name:'Enregistrer le paiement',exact:true}).click(); }
        assert.equal(await payment.locator('[name=amount]').inputValue(), '');
        await payment.locator('[name=amount]').fill('100');
        await payment.locator('summary').filter({hasText:'Référence et note'}).click();
        ...
        await savePayment();
        await payment.getByText(/^Le paiement n’a pas pu être confirmé/).waitFor();
        await payment.getByRole('button',{name:'Modifier',exact:true}).click();
        ...
        await page.getByRole('dialog', { name: 'Émettre le solde', exact: true }).getByRole('button', { name: 'Confirmer et émettre le solde', exact: true }).click();
