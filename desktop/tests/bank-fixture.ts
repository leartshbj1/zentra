...
export function installBankFixture(getWorkspace: () => Workspace) {
  ...
  workspace.accountingSettings!.enabled = !new URLSearchParams(location.search).has('bankNoAccounting');
  ...
  let linked = !new URLSearchParams(location.search).has('bankUnlinked');
  let holdResolve: (() => void) | undefined;
  (window as unknown as { bankFixture: { release: () => void } }).bankFixture = { release: () => { holdResolve?.(); holdResolve = undefined; } };
  desktopApi.associateBankAccount = async () => { linked = true; };
  desktopApi.dissociateBankAccount = async () => { linked = false; };
  ...
  function bankSnapshot() {
    ...
    return bankWorkspaceFromRaw({ summary: { import_count: hasImport ? 1 : 0, movement_count: active.length, unreconciled_count: open.filter((row) => row.credit_debit==='CRDT').length, unreconciled_supplier_count: open.filter((row) => row.credit_debit==='DBIT').length, pending_count: active.filter((row) => row.status==='PDNG').length, booked_credit_count: active.filter((row) => row.status==='BOOK' && row.credit_debit==='CRDT').length, booked_debit_count: active.filter((row) => row.status==='BOOK' && row.credit_debit==='DBIT').length }, accounts: hasImport ? [{ account_id: account, currency: 'CHF', linked, link_source: new URLSearchParams(location.search).has('bankUnlinked') ? (linked ? 'explicit' : 'none') : 'settings_iban', movement_count: active.length }] : [], movements: active, imports: hasImport ? [imported] : [], reconciliations: active.flatMap((row) => row.reconciliation ? [row.reconciliation] : []), supplier_reconciliations: active.flatMap((row) => row.supplier_reconciliation ? [row.supplier_reconciliation] : []) });
  }
  ...
  desktopApi.chooseCamtFile = async () => sessionStorage.getItem('qa-bank-cancel-file') === '1' ? null : 'releve-recette.xml';
  desktopApi.importCamtFile = async (_path, automatic) => {
    ...
    if (sessionStorage.getItem('qa-bank-reject-file') === '1') throw new Error('Le namespace réel de Document doit être camt.053 ou camt.054, version .001.04 ou .001.08.');
    const duplicate = hasImport;
    hasImport = true;
    sessionStorage.setItem('qa-bank-import-automatic', String(automatic));
    if (automatic && linked) { customerPayment('full','full'); customerPayment('partial','partial'); }
  ...
  desktopApi.confirmBankReconciliation = async (id,invoiceId) => {
    ...
    if (sessionStorage.getItem('qa-bank-hold-payment') === '1') await new Promise<void>(resolve => { holdResolve = resolve; });
