...
import { installBusinessResolutionFixture } from './business-resolution-fixture';
...
if (new URLSearchParams(location.search).has('bankRefundCreate')) installBankRefundCreateFixture(data);
...
if (new URLSearchParams(location.search).has('readOnlyAudit')) { installPayrollFixture(data); installReadOnlyFixture(data); }
if (new URLSearchParams(location.search).has('wizard')) {
  ...
  desktopApi.createEntity = async (entity, input) => {
    if (entity !== 'clients') throw new Error('Only synthetic clients may be created in this fixture.');
    data.clients.push(input as never);
    sessionStorage.setItem('wizard-client', JSON.stringify(input));
    return structuredClone(data);
  };
}
...
if (cycleAccount && new URLSearchParams(location.search).has('businessResolution')) installBusinessResolutionFixture(() => data);
function Harness() {
  useMobileLayout();
