...
const features = [
  { icon: FolderKanban, title: 'Chantiers & clients', text: 'Dates prévues et réelles, budget, avancement, documents, intervenants et notes au même endroit.' },
  ...
  { icon: Receipt, title: 'Factures & paiements', text: 'Acomptes, situations, factures finales, échéances et montants réellement encaissés.' },
  { icon: Clock3, title: 'Temps de travail', text: 'Chronomètre ou saisie manuelle, par collaborateur et par chantier, avec historique vérifiable.' },
...
const localPromises = [
  ...
  { icon: LockKeyhole, title: 'Aucun espace cloud métier', text: 'L’application n’envoie pas vos données de chantier vers un serveur HelviChantier.' },
];
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#f6f4ef] text-[#18221d]">
      <header className="mx-auto flex w-full max-w-7xl items-center justify-between px-5 py-5 lg:px-8">
        ...
        <Link href="/telecharger" className={cn(buttonVariants({ size: 'lg' }), 'rounded-full bg-[#173d2c] px-5 text-white hover:bg-[#24563f]')}>
      ...
      <section id="accueil" className="mx-auto grid w-full max-w-7xl gap-12 px-5 pb-20 pt-14 lg:grid-cols-[.9fr_1.1fr] lg:items-center lg:px-8 lg:pb-28 lg:pt-20">
        <div className="relative z-10">
          ...
          <h1 className="max-w-xl text-balance text-5xl font-semibold leading-[.98] tracking-[-.055em] sm:text-6xl lg:text-7xl">
            Chaque chantier.<br /><span className="text-[#b86b16]">Chaque franc.</span><br />Enfin clair.
          </h1>
          <p className="mt-7 max-w-xl text-lg leading-8 text-[#667068]">
            Une vraie application Windows pour vos devis, factures, heures, dépenses, chantiers et salaires préparatoires. Vos données professionnelles restent sur votre PC.
          </p>
          <div className="mt-9 flex flex-col gap-3 sm:flex-row">
            <Link href="/telecharger" className={cn(buttonVariants({ size: 'lg' }), 'h-12 rounded-full bg-[#e79b2f] px-6 text-[#1f281f] shadow-[0_10px_30px_rgba(201,117,21,.2)] hover:bg-[#f1aa42]')}>
          ...
          <div className="overflow-hidden rounded-[28px] border border-black/10 bg-[#fffdf8] shadow-[0_35px_90px_rgba(35,45,38,.16)]">
            ...
            <div className="grid min-h-[490px] grid-cols-[74px_1fr] sm:grid-cols-[170px_1fr]">
              <aside className="border-r border-[#e9e5dc] bg-[#173d2c] p-3 text-white sm:p-5">
                ...
                {['Entreprise', 'Facturation', 'Chantiers', 'Salaires', 'Sauvegarde'].map((item, index) => (
              ...
              <div className="min-w-0 p-5 sm:p-7">
                <div className="flex items-center justify-between text-[11px] font-medium text-[#778079]"><span>QUESTIONNAIRE DE CONFIGURATION</span><span>1 / 5</span></div>
                <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-[#e8e5dd]"><div className="h-full w-1/5 rounded-full bg-[#e79b2f]" /></div>
                <h2 className="mt-8 text-2xl font-semibold tracking-tight">Parlez-nous de votre entreprise</h2>
                <p className="mt-2 text-sm leading-6 text-[#747d76]">Aucune société ni donnée d’exemple n’est créée. Vous renseignez uniquement vos informations réelles.</p>
                <div className="mt-7 grid gap-4 sm:grid-cols-2">
                  {['Raison sociale *', 'Forme juridique *', 'Nom du responsable *', 'E-mail professionnel *', 'Rue et numéro *', 'NPA et localité *'].map((label, index) => (
                    <label key={label} className={index > 3 ? 'sm:col-span-2' : ''}><span className="text-[11px] font-medium text-[#606a63]">{label}</span><span className="mt-1.5 block h-10 rounded-xl border border-[#dedbd2] bg-white" /></label>
                  ))}
                </div>
      ...
      <section id="fonctionnalites" className="border-b border-[#ded9ce] bg-[#fffdf9] px-5 py-24 lg:px-8">
        <div className="mx-auto max-w-7xl">
          <div className="grid gap-8 lg:grid-cols-[.75fr_1.25fr] lg:items-end"><div><p className="text-xs font-semibold uppercase tracking-[.13em] text-[#9a651f]">Du devis au bilan</p><h2 className="mt-4 text-4xl font-semibold leading-tight tracking-[-.045em] sm:text-5xl">Un vrai outil de gestion, pas une vitrine vide.</h2></div><p className="max-w-2xl text-lg leading-8 text-[#6b746e] lg:justify-self-end">Les indicateurs apparaissent seulement lorsque vous avez saisi les données nécessaires. Aucun client, montant, chantier ou salaire fictif n’est injecté.</p></div>
      ...
      <section className="px-5 py-24 lg:px-8">
        <div className="mx-auto max-w-7xl">
          <div className="max-w-3xl"><p className="text-xs font-semibold uppercase tracking-[.13em] text-[#9a651f]">Votre première ouverture</p><h2 className="mt-4 text-4xl font-semibold tracking-[-.045em] sm:text-5xl">Le logiciel s’adapte à votre entreprise.</h2><p className="mt-5 text-lg leading-8 text-[#6b746e]">L’assistant demande les informations nécessaires avant d’autoriser la facturation. Vous pouvez aussi restaurer directement une sauvegarde existante.</p></div>
          <div className="mt-14 grid gap-4 lg:grid-cols-4">{[
            ['01', 'Identité', 'Raison sociale, responsable, adresse, UID, coordonnées et logo.'],
            ['02', 'Facturation', 'IBAN, numérotation, délais, validité et vos taux de TVA explicites.'],
            ['03', 'Organisation', 'Temps de travail, catégories de coûts et module de salaire facultatif.'],
            ['04', 'Protection', 'Emplacement des données, sauvegarde et récapitulatif avant création.'],
      ...
      <section className="border-y border-[#ded9ce] bg-[#fffdf9] px-5 py-24 lg:px-8">
        <div className="mx-auto grid max-w-7xl gap-14 lg:grid-cols-[.85fr_1.15fr] lg:items-center">
          <div><p className="text-xs font-semibold uppercase tracking-[.13em] text-[#9a651f]">Des chiffres explicables</p><h2 className="mt-4 text-4xl font-semibold leading-tight tracking-[-.045em] sm:text-5xl">Savoir combien un chantier a duré et ce qu’il a rapporté.</h2><p className="mt-6 text-lg leading-8 text-[#6b746e]">Le logiciel sépare durée prévue, dates réelles et heures travaillées. La rentabilité utilise uniquement le facturé net, les coûts horaires que vous avez configurés et les dépenses enregistrées.</p></div>
          <div className="grid gap-3 sm:grid-cols-2">{[
            { icon: Banknote, title: 'Facturé et encaissé', text: 'Deux montants distincts, par facture et par chantier.' },
            { icon: Clock3, title: 'Durée et heures réelles', text: 'Calendrier du chantier et temps pointé ne sont jamais confondus.' },
      ...
      <section id="tarif" className="px-5 py-24 lg:px-8">
        <div className="mx-auto max-w-5xl">
          ...
          <div className="mt-12 grid overflow-hidden rounded-[28px] border border-[#d9d4c9] bg-white shadow-[0_25px_70px_rgba(29,45,35,.1)] md:grid-cols-[1.1fr_.9fr]">
            <div className="p-7 sm:p-10"><div className="flex items-end gap-2"><span className="text-5xl font-semibold tracking-[-.05em]">50 CHF</span><span className="pb-1 text-sm text-[#767f78]">/ mois</span></div><p className="mt-3 text-sm text-[#747d76]">Hors TVA, le cas échéant. Conditions d’activation communiquées lors de la commande.</p><div className="mt-8 grid gap-3 sm:grid-cols-2">{['Application Windows complète', 'Données métier locales', 'Chantiers & clients', 'Devis & factures', 'Paiements & échéances', 'Temps & dépenses', 'Rentabilité par chantier', 'Salaires préparatoires', 'Sauvegarde & restauration', 'Mises à jour incluses'].map((item) => <div key={item} className="flex items-center gap-2 text-sm"><Check className="size-4 text-[#3f7454]" />{item}</div>)}</div></div>
            <div className="flex flex-col justify-between bg-[#173d2c] p-7 text-white sm:p-10"><div><Laptop className="size-7 text-[#efaa3c]" /><h3 className="mt-6 text-2xl font-semibold tracking-tight">Une vraie application `.exe`.</h3><p className="mt-4 text-sm leading-6 text-white/55">Téléchargez l’installateur, terminez le questionnaire puis commencez avec une base totalement vide.</p></div><div className="mt-9 space-y-3"><Link href="/telecharger" className="flex h-12 items-center justify-center gap-2 rounded-full bg-[#efaa3c] px-5 text-sm font-semibold text-[#173d2c]">Télécharger pour Windows <FileDown className="size-4" /></Link><a href="mailto:leartshabija@gmail.com?subject=Activer%20HelviChantier" className="flex h-12 items-center justify-center rounded-full border border-white/15 px-5 text-sm font-semibold text-white">Contacter le service commercial</a></div></div>
          </div>
        </div>
      </section>
