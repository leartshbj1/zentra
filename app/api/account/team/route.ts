...
export async function GET(request: Request) {
  try {
    ...
    const members = manage ? await db.prepare('SELECT membership_id AS id,email,display_name AS displayName,role FROM organization_members WHERE organization_id=? AND revoked_at IS NULL ORDER BY joined_at').bind(session.organizationId).all() : { results:[] };
