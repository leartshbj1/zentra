      ...
      <div className="relative mx-auto grid w-full max-w-5xl gap-10 lg:grid-cols-[1fr_28rem] lg:items-center lg:gap-16">
        <section className="pt-3 lg:pt-0">
          <a href="/" className="inline-flex items-center gap-3 font-semibold">
            <span className="grid size-11 place-items-center rounded-2xl bg-[#173d2c] text-lg font-bold text-white">Z</span>
            <span className="text-xl tracking-[-.03em]">zentra</span>
          </a>
          <p className="mt-12 text-xs font-bold uppercase tracking-[.2em] text-[#a66b1f]">Compte d’entreprise</p>
          ...
          <div className="mt-8 grid gap-3 sm:grid-cols-3 lg:grid-cols-1">
            {[
              [ShieldCheck, 'Session protégée', 'Cookies HttpOnly et renouvellement contrôlé.'],
              [Building2, 'Équipe sans supplément', 'Collaborateurs et comptable dans la même entreprise.'],
              [DatabaseZap, 'Données maîtrisées', 'Aucun jeu de données fictif dans votre espace.'],
            ].map(([Icon, title, description]) => {
              const ItemIcon = Icon as typeof ShieldCheck;
              return (
                <div key={String(title)} className="flex items-start gap-3 rounded-2xl border border-[#d9ddd6] bg-white/65 p-4 backdrop-blur-sm">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-xs leading-5 text-[#6b766f]">{String(description)}</p>
                  </div>
                </div>
              );
            })}
