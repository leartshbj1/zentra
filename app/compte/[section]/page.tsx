...
import { SubscriptionOverview } from '@/components/account/subscription-overview';
...
export default async function Page({params,searchParams}:{params:Promise<{section:string}>;searchParams:Promise<Record<string,string|string[]|undefined>>}) {
  ...
  if(section==='abonnement') return <>{head}<SubscriptionOverview organizationId={org.organizationId} initial={await subscriptionJourney({...org,userId:user.userId,founder:false})}/></>;
