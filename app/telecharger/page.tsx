import { ArrowLeft, Check, Download, FileArchive, HardDrive, Laptop, ShieldAlert, ShieldCheck, WifiOff } from 'lucide-react';
import { BrandMark } from '@/components/brand-mark';
import { PurchaseButton } from '@/components/purchase-button';

export const metadata = {
  title: 'Télécharger Elyko pour Windows',
  description: 'Téléchargez le véritable installateur Windows .exe multisectoriel d’Elyko.',
  openGraph: {
    title: 'Télécharger Elyko pour Windows',
...
const installerPath = '/downloads/Elyko_1.1.0_x64-setup.exe';
  ...
  return (
    <main className="min-h-screen bg-[#f4f2ed] text-[#17231d]">
      <header className="mx-auto flex max-w-6xl items-center justify-between px-5 py-5 lg:px-8">
        <a href="/" className="flex items-center gap-2.5"><BrandMark className="size-9" /><span className="font-semibold tracking-[-.03em]">Elyko</span></a>
      ...
      <section className="mx-auto grid max-w-6xl gap-12 px-5 pb-20 pt-14 lg:grid-cols-[.9fr_1.1fr] lg:items-center lg:px-8 lg:pt-24">
        <div>
          ...
          <p className="mt-6 max-w-lg text-lg leading-8 text-[#67716a]">Téléchargez le fichier `.exe`, installez Elyko puis renseignez votre entreprise et son activité. La première base contient exactement zéro client, zéro projet, zéro chantier et zéro montant.</p>
          <div className="mt-8 max-w-md"><PurchaseButton /></div>
          <a href={installerPath} download className="mt-4 inline-flex h-13 items-center justify-center gap-3 rounded-full border border-[#b8b3a8] bg-white px-7 text-sm font-semibold text-[#173d2c] hover:bg-[#fffdf8]"><Download className="size-5" /> Télécharger Elyko 1.1.0</a>
          <div className="mt-5 space-y-1 text-xs leading-5 text-[#7f8781]"><p>Format : installateur Windows `.exe` · application x64 · 2,05 Mio</p><p>Compatibilité : Windows 10 et Windows 11</p><p>SHA-256 : <code className="break-all">17E83B5341AE7E546ED11AE7D4290CCD61482ABE915342ADCD74B6B99EF4FEE0</code> · <a className="underline underline-offset-2" href={`${installerPath}.sha256.txt`}>fichier de contrôle</a></p><p>Tarif encaissé par Stripe : 50 CHF par mois</p></div>
