...
import { Check, Copy, CreditCard, Download, KeyRound, LoaderCircle } from 'lucide-react';
import { useState, type SyntheticEvent } from 'react';

type LicenseResponse = {
  token?: string;
  payload?: { customer_name?: string | null; valid_until?: string; installation_id?: string };
  error?: string;
};
...
export function LicenseDelivery({ sessionId }: { sessionId: string }) {
  ...
  async function activate(event: SyntheticEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch('/api/stripe/license', {
        ...
        body: JSON.stringify({ sessionId, installationId: installationId.trim() }),
      });
      const body = (await response.json()) as LicenseResponse;
      if (!response.ok || !body.token) throw new Error(body.error || 'La licence n’a pas pu être émise.');
      setToken(body.token);
      setValidUntil(body.payload?.valid_until ?? '');
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : 'La licence n’a pas pu être émise.');
    } finally {
      setBusy(false);
    }
  }
  ...
  async function openPortal() {
    ...
    try {
      ...
      if (!response.ok || !body.url) throw new Error(body.error || 'Le portail Stripe n’est pas disponible.');
      window.location.assign(body.url);
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : 'Le portail Stripe n’est pas disponible.');
      setPortalBusy(false);
    }
  }
  ...
  return (
    <div className="rounded-[28px] border border-[#d9d4c9] bg-white p-6 shadow-[0_25px_70px_rgba(29,45,35,.1)] sm:p-9">
      <div className="flex items-start gap-3">
        <span className="grid size-11 shrink-0 place-items-center rounded-2xl bg-[#e7efe9] text-[#315d47]"><Check className="size-5" /></span>
        <div><h2 className="text-xl font-semibold">Paiement reçu</h2><p className="mt-1 text-sm leading-6 text-[#6f7971]">La licence est liée à une seule installation Windows afin de limiter le partage.</p></div>
      </div>
      {!token ? (
      ...
      <div className="mt-6 grid gap-3 sm:grid-cols-2">
        <a href="/telecharger" className="inline-flex h-11 items-center justify-center gap-2 rounded-full bg-[#e7a33a] px-4 text-sm font-semibold text-[#173d2c]"><Download className="size-4" /> Télécharger le .exe</a>
        <button type="button" onClick={() => void openPortal()} disabled={portalBusy} className="inline-flex h-11 items-center justify-center gap-2 rounded-full border border-[#cbc7bd] px-4 text-sm font-semibold disabled:opacity-60">{portalBusy ? <LoaderCircle className="size-4 animate-spin" /> : <CreditCard className="size-4" />} Gérer l’abonnement</button>
      </div>
    </div>
  );
}
