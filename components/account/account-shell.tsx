  ...
  return <div className={`account-experience ${isIndex ? 'account-index' : 'account-detail'} ${!organizations.length ? 'account-onboarding' : ''}`} data-theme={theme}>
    ...
    <div className="account-workspace">
      <aside className="account-sidebar" aria-label="Paramètres du compte">
        ...
        <nav>{ACCOUNT_SECTIONS.map(item => <a key={item.id} href={`/compte/${item.id}${query}`} aria-current={pathname === `/compte/${item.id}` || pathname.startsWith(`/compte/${item.id}/`) ? 'page' : undefined}><item.icon size={20}/><span><strong>{item.label}</strong><small>{item.detail}</small></span><ChevronRight size={16} className="account-nav-chevron"/></a>)}</nav>
