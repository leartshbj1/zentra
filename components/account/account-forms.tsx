...
export function CompanyDraftForm({preferences}:{preferences:AccountPreferences}) {
  return <AccountForm endpoint="/api/account/trial" method="POST" values={{legalVersion:LEGAL_VERSION}} success="Votre entreprise est prête. Votre essai de 14 jours a commencé." label="Créer mon entreprise et commencer l’essai"><label>Nom de votre entreprise<input name="companyName" defaultValue={preferences.companyDraft} autoComplete="organization" required maxLength={120}/></label><p className="account-caption">14 jours de Zentra Gestion Solo, pour une personne. Sans engagement et sans carte bancaire. Aucun paiement automatique à la fin de l’essai.</p><label className="account-consent"><input name="acceptTerms" type="checkbox" required/><span>J’accepte les <a href="/conditions" target="_blank" rel="noopener noreferrer">conditions d’utilisation</a>.</span></label></AccountForm>;
}
