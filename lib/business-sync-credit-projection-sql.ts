...
// The arithmetic is identical for initial histories and subsequent candidates.
// Only their trusted, server-built publication guards differ.
export function creditProjectionQueries(active: string) {
 ...
 AND p.manifest_sha256=a.manifest_sha256 AND p.generation=a.generation AND p.revision=a.revision AND p.state_json=a.state_json AND EXISTS(${active}))`;
  ...
  return {
    ...
    linePage: query(
      'SELECT item_id FROM page_lines ORDER BY item_id',
      pageLines,
    ),
    gross: query(
...
export const creditProjectionSql = creditProjectionQueries(active);
