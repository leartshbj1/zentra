...
vi.mock('@/app/zentra-auth', () => ({
  getZentraUser: mocks.user,
  zentraSignInPath: () => '/connexion',
}));
vi.mock('@/lib/account', () => ({
  requireBrowserMembership: mocks.membership,
}));
vi.mock('@/lib/supplier-inbox/service', () => ({ inboxState: mocks.inbox }));
vi.mock('@/components/automation/supplier-receipts', () => ({
  SupplierReceipts: () => null,
}));
...
describe('supplier reception page company boundary', () => {
  ...
  it('requires authentication before reading documents', async () => {
    ...
    await expect(
      Page({ searchParams: Promise.resolve({ entreprise: 'company-a' }) }),
  ...
  it('never reads another company after membership is refused', async () => {
    ...
    await expect(
      Page({ searchParams: Promise.resolve({ entreprise: 'company-b' }) }),
